...
/-!
# Term-structural certificate sources: `Magma.end7A`

For each of the following magmas, drawn from the `2` magmas on `Fin 7` whose endomorphism monoid is
exactly the order-`675` monoid of `Magma.endE7A`, the equations it satisfies that no earlier listed
magma does. Together they are the `133` equations that are worth using as the *source* `L'` of a
`Magma.end7A` certificate -- the rest have such a model too, but the resulting facts are already
...
/-- `73` equations satisfied by `Magma.end7A 0`, and by no earlier member of the cover. -/
theorem end7A_satisfies_0 :
    Satisfies (Magma.end7A 0) [
...
/-- `60` equations satisfied by `Magma.end7A 1`, and by no earlier member of the cover. -/
theorem end7A_satisfies_1 :
    Satisfies (Magma.end7A 1) [
