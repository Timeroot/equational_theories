import equational_theories.Definability.S4351

/-!
# `StructuralFrom` the square-column family, batch `N`

Generated by `defwork/gemit51.py`.  Each of these reads a source of the square-column family back
out of an operation of `Definability/S4351.lean`.
-/

set_option linter.unusedVariables false
set_option linter.unusedSimpArgs false
set_option maxErrors 10000
set_option maxHeartbeats 40000000
set_option linter.unusedTactic false
set_option linter.unreachableTactic false
-- a failing goal here is a tower of nested `if`s several hundred lines deep, and a batch of them
-- runs to ninety megabytes of log that nothing reads; the harvest only wants the line numbers
set_option pp.deepTerms false
set_option pp.maxSteps 40

open Law Law.MagmaLaw Quad4274 Quad4374 Quad4351 Sq4351

namespace Law.MagmaLaw

/-- `x ◇ y = (x ◇ x) ◇ (x ◇ y)` -/
theorem Equation3712_StructuralFrom_Equation3479 : Law3712.StructuralFrom Law3479 := by
  intro G M hM
  have h : Equation3479 G := Law3479.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3479 h)⟩
  refine structural_q5132319 M K ?_
  rw [@Law3712.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     grind [q5132319_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q5132319_dg M K, K.dsq]
     try simp only [q5132319_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q5132319_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q5132319_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind [q5132319_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q5132319_apply M K])

/-- `x ◇ y = (x ◇ y) ◇ (x ◇ x)` -/
theorem Equation3721_StructuralFrom_Equation3479 : Law3721.StructuralFrom Law3479 := by
  intro G M hM
  have h : Equation3479 G := Law3479.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3479 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law3721.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x ◇ y = (x ◇ y) ◇ (z ◇ z)` -/
theorem Equation3729_StructuralFrom_Equation3479 : Law3729.StructuralFrom Law3479 := by
  intro G M hM
  have h : Equation3479 G := Law3479.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3479 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law3729.models_iff]
  intro x y z
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x z
     have e4 := h x x (M.op x x)
     have e5 := h x x (M.op y y)
     have e6 := h x y x
     have e7 := h x y y
     have e8 := h x y z
     have e9 := h x y (M.op x x)
     have e10 := h x y (M.op y y)
     have e11 := h x z x
     have e12 := h x z y
     have e13 := h x z z
     have e14 := h x z (M.op x x)
     have e15 := h x z (M.op y y)
     have e16 := h x (M.op x x) x
     have e17 := h x (M.op x x) y
     have e18 := h x (M.op x x) z
     have e19 := h x (M.op x x) (M.op x x)
     have e20 := h x (M.op x x) (M.op y y)
     have e21 := h x (M.op y y) x
     have e22 := h x (M.op y y) y
     have e23 := h x (M.op y y) z
     have e24 := h x (M.op y y) (M.op x x)
     have e25 := h x (M.op y y) (M.op y y)
     have e26 := h y x x
     have e27 := h y x y
     have e28 := h y x z
     have e29 := h y x (M.op x x)
     have e30 := h y x (M.op y y)
     have e31 := h y y x
     have e32 := h y y y
     have e33 := h y y z
     have e34 := h y y (M.op x x)
     have e35 := h y y (M.op y y)
     have e36 := h y z x
     have e37 := h y z y
     have e38 := h y z z
     have e39 := h y z (M.op x x)
     have e40 := h y z (M.op y y)
     have e41 := h y (M.op x x) x
     have e42 := h y (M.op x x) y
     have e43 := h y (M.op x x) z
     have e44 := h y (M.op x x) (M.op x x)
     have e45 := h y (M.op x x) (M.op y y)
     have e46 := h y (M.op y y) x
     have e47 := h y (M.op y y) y
     have e48 := h y (M.op y y) z
     have e49 := h y (M.op y y) (M.op x x)
     have e50 := h y (M.op y y) (M.op y y)
     have e51 := h z x x
     have e52 := h z x y
     have e53 := h z x z
     have e54 := h z x (M.op x x)
     have e55 := h z x (M.op y y)
     have e56 := h z y x
     have e57 := h z y y
     have e58 := h z y z
     have e59 := h z y (M.op x x)
     have e60 := h z y (M.op y y)
     have e61 := h z z x
     have e62 := h z z y
     have e63 := h z z z
     have e64 := h z z (M.op x x)
     have e65 := h z z (M.op y y)
     have e66 := h z (M.op x x) x
     have e67 := h z (M.op x x) y
     have e68 := h z (M.op x x) z
     have e69 := h z (M.op x x) (M.op x x)
     have e70 := h z (M.op x x) (M.op y y)
     have e71 := h z (M.op y y) x
     have e72 := h z (M.op y y) y
     have e73 := h z (M.op y y) z
     have e74 := h z (M.op y y) (M.op x x)
     have e75 := h z (M.op y y) (M.op y y)
     have e76 := h (M.op x x) x x
     have e77 := h (M.op x x) x y
     have e78 := h (M.op x x) x z
     have e79 := h (M.op x x) x (M.op x x)
     have e80 := h (M.op x x) x (M.op y y)
     have e81 := h (M.op x x) y x
     have e82 := h (M.op x x) y y
     have e83 := h (M.op x x) y z
     have e84 := h (M.op x x) y (M.op x x)
     have e85 := h (M.op x x) y (M.op y y)
     have e86 := h (M.op x x) z x
     have e87 := h (M.op x x) z y
     have e88 := h (M.op x x) z z
     have e89 := h (M.op x x) z (M.op x x)
     have e90 := h (M.op x x) z (M.op y y)
     have e91 := h (M.op x x) (M.op x x) x
     have e92 := h (M.op x x) (M.op x x) y
     have e93 := h (M.op x x) (M.op x x) z
     have e94 := h (M.op x x) (M.op x x) (M.op x x)
     have e95 := h (M.op x x) (M.op x x) (M.op y y)
     have e96 := h (M.op x x) (M.op y y) x
     have e97 := h (M.op x x) (M.op y y) y
     have e98 := h (M.op x x) (M.op y y) z
     have e99 := h (M.op x x) (M.op y y) (M.op x x)
     have e100 := h (M.op x x) (M.op y y) (M.op y y)
     have e101 := h (M.op y y) x x
     have e102 := h (M.op y y) x y
     have e103 := h (M.op y y) x z
     have e104 := h (M.op y y) x (M.op x x)
     have e105 := h (M.op y y) x (M.op y y)
     have e106 := h (M.op y y) y x
     have e107 := h (M.op y y) y y
     have e108 := h (M.op y y) y z
     have e109 := h (M.op y y) y (M.op x x)
     have e110 := h (M.op y y) y (M.op y y)
     have e111 := h (M.op y y) z x
     have e112 := h (M.op y y) z y
     have e113 := h (M.op y y) z z
     have e114 := h (M.op y y) z (M.op x x)
     have e115 := h (M.op y y) z (M.op y y)
     have e116 := h (M.op y y) (M.op x x) x
     have e117 := h (M.op y y) (M.op x x) y
     have e118 := h (M.op y y) (M.op x x) z
     have e119 := h (M.op y y) (M.op x x) (M.op x x)
     have e120 := h (M.op y y) (M.op x x) (M.op y y)
     have e121 := h (M.op y y) (M.op y y) x
     have e122 := h (M.op y y) (M.op y y) y
     have e123 := h (M.op y y) (M.op y y) z
     have e124 := h (M.op y y) (M.op y y) (M.op x x)
     have e125 := h (M.op y y) (M.op y y) (M.op y y)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x z
     have e4 := h x x (M.op x x)
     have e5 := h x x (M.op y y)
     have e6 := h x y x
     have e7 := h x y y
     have e8 := h x y z
     have e9 := h x y (M.op x x)
     have e10 := h x y (M.op y y)
     have e11 := h x z x
     have e12 := h x z y
     have e13 := h x z z
     have e14 := h x z (M.op x x)
     have e15 := h x z (M.op y y)
     have e16 := h x (M.op x x) x
     have e17 := h x (M.op x x) y
     have e18 := h x (M.op x x) z
     have e19 := h x (M.op x x) (M.op x x)
     have e20 := h x (M.op x x) (M.op y y)
     have e21 := h x (M.op y y) x
     have e22 := h x (M.op y y) y
     have e23 := h x (M.op y y) z
     have e24 := h x (M.op y y) (M.op x x)
     have e25 := h x (M.op y y) (M.op y y)
     have e26 := h y x x
     have e27 := h y x y
     have e28 := h y x z
     have e29 := h y x (M.op x x)
     have e30 := h y x (M.op y y)
     have e31 := h y y x
     have e32 := h y y y
     have e33 := h y y z
     have e34 := h y y (M.op x x)
     have e35 := h y y (M.op y y)
     have e36 := h y z x
     have e37 := h y z y
     have e38 := h y z z
     have e39 := h y z (M.op x x)
     have e40 := h y z (M.op y y)
     have e41 := h y (M.op x x) x
     have e42 := h y (M.op x x) y
     have e43 := h y (M.op x x) z
     have e44 := h y (M.op x x) (M.op x x)
     have e45 := h y (M.op x x) (M.op y y)
     have e46 := h y (M.op y y) x
     have e47 := h y (M.op y y) y
     have e48 := h y (M.op y y) z
     have e49 := h y (M.op y y) (M.op x x)
     have e50 := h y (M.op y y) (M.op y y)
     have e51 := h z x x
     have e52 := h z x y
     have e53 := h z x z
     have e54 := h z x (M.op x x)
     have e55 := h z x (M.op y y)
     have e56 := h z y x
     have e57 := h z y y
     have e58 := h z y z
     have e59 := h z y (M.op x x)
     have e60 := h z y (M.op y y)
     have e61 := h z z x
     have e62 := h z z y
     have e63 := h z z z
     have e64 := h z z (M.op x x)
     have e65 := h z z (M.op y y)
     have e66 := h z (M.op x x) x
     have e67 := h z (M.op x x) y
     have e68 := h z (M.op x x) z
     have e69 := h z (M.op x x) (M.op x x)
     have e70 := h z (M.op x x) (M.op y y)
     have e71 := h z (M.op y y) x
     have e72 := h z (M.op y y) y
     have e73 := h z (M.op y y) z
     have e74 := h z (M.op y y) (M.op x x)
     have e75 := h z (M.op y y) (M.op y y)
     have e76 := h (M.op x x) x x
     have e77 := h (M.op x x) x y
     have e78 := h (M.op x x) x z
     have e79 := h (M.op x x) x (M.op x x)
     have e80 := h (M.op x x) x (M.op y y)
     have e81 := h (M.op x x) y x
     have e82 := h (M.op x x) y y
     have e83 := h (M.op x x) y z
     have e84 := h (M.op x x) y (M.op x x)
     have e85 := h (M.op x x) y (M.op y y)
     have e86 := h (M.op x x) z x
     have e87 := h (M.op x x) z y
     have e88 := h (M.op x x) z z
     have e89 := h (M.op x x) z (M.op x x)
     have e90 := h (M.op x x) z (M.op y y)
     have e91 := h (M.op x x) (M.op x x) x
     have e92 := h (M.op x x) (M.op x x) y
     have e93 := h (M.op x x) (M.op x x) z
     have e94 := h (M.op x x) (M.op x x) (M.op x x)
     have e95 := h (M.op x x) (M.op x x) (M.op y y)
     have e96 := h (M.op x x) (M.op y y) x
     have e97 := h (M.op x x) (M.op y y) y
     have e98 := h (M.op x x) (M.op y y) z
     have e99 := h (M.op x x) (M.op y y) (M.op x x)
     have e100 := h (M.op x x) (M.op y y) (M.op y y)
     have e101 := h (M.op y y) x x
     have e102 := h (M.op y y) x y
     have e103 := h (M.op y y) x z
     have e104 := h (M.op y y) x (M.op x x)
     have e105 := h (M.op y y) x (M.op y y)
     have e106 := h (M.op y y) y x
     have e107 := h (M.op y y) y y
     have e108 := h (M.op y y) y z
     have e109 := h (M.op y y) y (M.op x x)
     have e110 := h (M.op y y) y (M.op y y)
     have e111 := h (M.op y y) z x
     have e112 := h (M.op y y) z y
     have e113 := h (M.op y y) z z
     have e114 := h (M.op y y) z (M.op x x)
     have e115 := h (M.op y y) z (M.op y y)
     have e116 := h (M.op y y) (M.op x x) x
     have e117 := h (M.op y y) (M.op x x) y
     have e118 := h (M.op y y) (M.op x x) z
     have e119 := h (M.op y y) (M.op x x) (M.op x x)
     have e120 := h (M.op y y) (M.op x x) (M.op y y)
     have e121 := h (M.op y y) (M.op y y) x
     have e122 := h (M.op y y) (M.op y y) y
     have e123 := h (M.op y y) (M.op y y) z
     have e124 := h (M.op y y) (M.op y y) (M.op x x)
     have e125 := h (M.op y y) (M.op y y) (M.op y y)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x ◇ (x ◇ x) = (x ◇ x) ◇ x` -/
theorem Equation4380_StructuralFrom_Equation3479 : Law4380.StructuralFrom Law3479 := by
  intro G M hM
  have h : Equation3479 G := Law3479.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3479 h)⟩
  refine structural_q8424930 M K ?_
  rw [@Law4380.models_iff]
  intro x
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     grind [q8424930_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q8424930_dg M K, K.dsq]
     try simp only [q8424930_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q8424930_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q8424930_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x (M.op x x)
     have e3 := h x (M.op x x) x
     have e4 := h x (M.op x x) (M.op x x)
     have e5 := h (M.op x x) x x
     have e6 := h (M.op x x) x (M.op x x)
     have e7 := h (M.op x x) (M.op x x) x
     have e8 := h (M.op x x) (M.op x x) (M.op x x)
     grind [q8424930_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x (M.op x x)
     have e3 := h x (M.op x x) x
     have e4 := h x (M.op x x) (M.op x x)
     have e5 := h (M.op x x) x x
     have e6 := h (M.op x x) x (M.op x x)
     have e7 := h (M.op x x) (M.op x x) x
     have e8 := h (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q8424930_apply M K])

/-- `x ◇ (x ◇ x) = (y ◇ y) ◇ x` -/
theorem Equation4388_StructuralFrom_Equation3479 : Law4388.StructuralFrom Law3479 := by
  intro G M hM
  have h : Equation3479 G := Law3479.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3479 h)⟩
  refine structural_q8424930 M K ?_
  rw [@Law4388.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     grind [q8424930_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q8424930_dg M K, K.dsq]
     try simp only [q8424930_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q8424930_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q8424930_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind [q8424930_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q8424930_apply M K])

/-- `x ◇ (y ◇ y) = (y ◇ y) ◇ x` -/
theorem Equation4482_StructuralFrom_Equation3479 : Law4482.StructuralFrom Law3479 := by
  intro G M hM
  have h : Equation3479 G := Law3479.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3479 h)⟩
  refine structural_q8424930 M K ?_
  rw [@Law4482.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     grind [q8424930_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q8424930_dg M K, K.dsq]
     try simp only [q8424930_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q8424930_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q8424930_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind [q8424930_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q8424930_apply M K])

/-- `x ◇ (y ◇ y) = (z ◇ z) ◇ x` -/
theorem Equation4497_StructuralFrom_Equation3479 : Law4497.StructuralFrom Law3479 := by
  intro G M hM
  have h : Equation3479 G := Law3479.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3479 h)⟩
  refine structural_q8424930 M K ?_
  rw [@Law4497.models_iff]
  intro x y z
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     grind [q8424930_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q8424930_dg M K, K.dsq]
     try simp only [q8424930_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q8424930_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q8424930_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x z
     have e4 := h x x (M.op x x)
     have e5 := h x x (M.op y y)
     have e6 := h x y x
     have e7 := h x y y
     have e8 := h x y z
     have e9 := h x y (M.op x x)
     have e10 := h x y (M.op y y)
     have e11 := h x z x
     have e12 := h x z y
     have e13 := h x z z
     have e14 := h x z (M.op x x)
     have e15 := h x z (M.op y y)
     have e16 := h x (M.op x x) x
     have e17 := h x (M.op x x) y
     have e18 := h x (M.op x x) z
     have e19 := h x (M.op x x) (M.op x x)
     have e20 := h x (M.op x x) (M.op y y)
     have e21 := h x (M.op y y) x
     have e22 := h x (M.op y y) y
     have e23 := h x (M.op y y) z
     have e24 := h x (M.op y y) (M.op x x)
     have e25 := h x (M.op y y) (M.op y y)
     have e26 := h y x x
     have e27 := h y x y
     have e28 := h y x z
     have e29 := h y x (M.op x x)
     have e30 := h y x (M.op y y)
     have e31 := h y y x
     have e32 := h y y y
     have e33 := h y y z
     have e34 := h y y (M.op x x)
     have e35 := h y y (M.op y y)
     have e36 := h y z x
     have e37 := h y z y
     have e38 := h y z z
     have e39 := h y z (M.op x x)
     have e40 := h y z (M.op y y)
     have e41 := h y (M.op x x) x
     have e42 := h y (M.op x x) y
     have e43 := h y (M.op x x) z
     have e44 := h y (M.op x x) (M.op x x)
     have e45 := h y (M.op x x) (M.op y y)
     have e46 := h y (M.op y y) x
     have e47 := h y (M.op y y) y
     have e48 := h y (M.op y y) z
     have e49 := h y (M.op y y) (M.op x x)
     have e50 := h y (M.op y y) (M.op y y)
     have e51 := h z x x
     have e52 := h z x y
     have e53 := h z x z
     have e54 := h z x (M.op x x)
     have e55 := h z x (M.op y y)
     have e56 := h z y x
     have e57 := h z y y
     have e58 := h z y z
     have e59 := h z y (M.op x x)
     have e60 := h z y (M.op y y)
     have e61 := h z z x
     have e62 := h z z y
     have e63 := h z z z
     have e64 := h z z (M.op x x)
     have e65 := h z z (M.op y y)
     have e66 := h z (M.op x x) x
     have e67 := h z (M.op x x) y
     have e68 := h z (M.op x x) z
     have e69 := h z (M.op x x) (M.op x x)
     have e70 := h z (M.op x x) (M.op y y)
     have e71 := h z (M.op y y) x
     have e72 := h z (M.op y y) y
     have e73 := h z (M.op y y) z
     have e74 := h z (M.op y y) (M.op x x)
     have e75 := h z (M.op y y) (M.op y y)
     have e76 := h (M.op x x) x x
     have e77 := h (M.op x x) x y
     have e78 := h (M.op x x) x z
     have e79 := h (M.op x x) x (M.op x x)
     have e80 := h (M.op x x) x (M.op y y)
     have e81 := h (M.op x x) y x
     have e82 := h (M.op x x) y y
     have e83 := h (M.op x x) y z
     have e84 := h (M.op x x) y (M.op x x)
     have e85 := h (M.op x x) y (M.op y y)
     have e86 := h (M.op x x) z x
     have e87 := h (M.op x x) z y
     have e88 := h (M.op x x) z z
     have e89 := h (M.op x x) z (M.op x x)
     have e90 := h (M.op x x) z (M.op y y)
     have e91 := h (M.op x x) (M.op x x) x
     have e92 := h (M.op x x) (M.op x x) y
     have e93 := h (M.op x x) (M.op x x) z
     have e94 := h (M.op x x) (M.op x x) (M.op x x)
     have e95 := h (M.op x x) (M.op x x) (M.op y y)
     have e96 := h (M.op x x) (M.op y y) x
     have e97 := h (M.op x x) (M.op y y) y
     have e98 := h (M.op x x) (M.op y y) z
     have e99 := h (M.op x x) (M.op y y) (M.op x x)
     have e100 := h (M.op x x) (M.op y y) (M.op y y)
     have e101 := h (M.op y y) x x
     have e102 := h (M.op y y) x y
     have e103 := h (M.op y y) x z
     have e104 := h (M.op y y) x (M.op x x)
     have e105 := h (M.op y y) x (M.op y y)
     have e106 := h (M.op y y) y x
     have e107 := h (M.op y y) y y
     have e108 := h (M.op y y) y z
     have e109 := h (M.op y y) y (M.op x x)
     have e110 := h (M.op y y) y (M.op y y)
     have e111 := h (M.op y y) z x
     have e112 := h (M.op y y) z y
     have e113 := h (M.op y y) z z
     have e114 := h (M.op y y) z (M.op x x)
     have e115 := h (M.op y y) z (M.op y y)
     have e116 := h (M.op y y) (M.op x x) x
     have e117 := h (M.op y y) (M.op x x) y
     have e118 := h (M.op y y) (M.op x x) z
     have e119 := h (M.op y y) (M.op x x) (M.op x x)
     have e120 := h (M.op y y) (M.op x x) (M.op y y)
     have e121 := h (M.op y y) (M.op y y) x
     have e122 := h (M.op y y) (M.op y y) y
     have e123 := h (M.op y y) (M.op y y) z
     have e124 := h (M.op y y) (M.op y y) (M.op x x)
     have e125 := h (M.op y y) (M.op y y) (M.op y y)
     grind [q8424930_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3479_sqeq h K
     have g2 := f3479_sqidem h K
     have g3 := f3479_sqfixl h K
     have g4 := f3479_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x z
     have e4 := h x x (M.op x x)
     have e5 := h x x (M.op y y)
     have e6 := h x y x
     have e7 := h x y y
     have e8 := h x y z
     have e9 := h x y (M.op x x)
     have e10 := h x y (M.op y y)
     have e11 := h x z x
     have e12 := h x z y
     have e13 := h x z z
     have e14 := h x z (M.op x x)
     have e15 := h x z (M.op y y)
     have e16 := h x (M.op x x) x
     have e17 := h x (M.op x x) y
     have e18 := h x (M.op x x) z
     have e19 := h x (M.op x x) (M.op x x)
     have e20 := h x (M.op x x) (M.op y y)
     have e21 := h x (M.op y y) x
     have e22 := h x (M.op y y) y
     have e23 := h x (M.op y y) z
     have e24 := h x (M.op y y) (M.op x x)
     have e25 := h x (M.op y y) (M.op y y)
     have e26 := h y x x
     have e27 := h y x y
     have e28 := h y x z
     have e29 := h y x (M.op x x)
     have e30 := h y x (M.op y y)
     have e31 := h y y x
     have e32 := h y y y
     have e33 := h y y z
     have e34 := h y y (M.op x x)
     have e35 := h y y (M.op y y)
     have e36 := h y z x
     have e37 := h y z y
     have e38 := h y z z
     have e39 := h y z (M.op x x)
     have e40 := h y z (M.op y y)
     have e41 := h y (M.op x x) x
     have e42 := h y (M.op x x) y
     have e43 := h y (M.op x x) z
     have e44 := h y (M.op x x) (M.op x x)
     have e45 := h y (M.op x x) (M.op y y)
     have e46 := h y (M.op y y) x
     have e47 := h y (M.op y y) y
     have e48 := h y (M.op y y) z
     have e49 := h y (M.op y y) (M.op x x)
     have e50 := h y (M.op y y) (M.op y y)
     have e51 := h z x x
     have e52 := h z x y
     have e53 := h z x z
     have e54 := h z x (M.op x x)
     have e55 := h z x (M.op y y)
     have e56 := h z y x
     have e57 := h z y y
     have e58 := h z y z
     have e59 := h z y (M.op x x)
     have e60 := h z y (M.op y y)
     have e61 := h z z x
     have e62 := h z z y
     have e63 := h z z z
     have e64 := h z z (M.op x x)
     have e65 := h z z (M.op y y)
     have e66 := h z (M.op x x) x
     have e67 := h z (M.op x x) y
     have e68 := h z (M.op x x) z
     have e69 := h z (M.op x x) (M.op x x)
     have e70 := h z (M.op x x) (M.op y y)
     have e71 := h z (M.op y y) x
     have e72 := h z (M.op y y) y
     have e73 := h z (M.op y y) z
     have e74 := h z (M.op y y) (M.op x x)
     have e75 := h z (M.op y y) (M.op y y)
     have e76 := h (M.op x x) x x
     have e77 := h (M.op x x) x y
     have e78 := h (M.op x x) x z
     have e79 := h (M.op x x) x (M.op x x)
     have e80 := h (M.op x x) x (M.op y y)
     have e81 := h (M.op x x) y x
     have e82 := h (M.op x x) y y
     have e83 := h (M.op x x) y z
     have e84 := h (M.op x x) y (M.op x x)
     have e85 := h (M.op x x) y (M.op y y)
     have e86 := h (M.op x x) z x
     have e87 := h (M.op x x) z y
     have e88 := h (M.op x x) z z
     have e89 := h (M.op x x) z (M.op x x)
     have e90 := h (M.op x x) z (M.op y y)
     have e91 := h (M.op x x) (M.op x x) x
     have e92 := h (M.op x x) (M.op x x) y
     have e93 := h (M.op x x) (M.op x x) z
     have e94 := h (M.op x x) (M.op x x) (M.op x x)
     have e95 := h (M.op x x) (M.op x x) (M.op y y)
     have e96 := h (M.op x x) (M.op y y) x
     have e97 := h (M.op x x) (M.op y y) y
     have e98 := h (M.op x x) (M.op y y) z
     have e99 := h (M.op x x) (M.op y y) (M.op x x)
     have e100 := h (M.op x x) (M.op y y) (M.op y y)
     have e101 := h (M.op y y) x x
     have e102 := h (M.op y y) x y
     have e103 := h (M.op y y) x z
     have e104 := h (M.op y y) x (M.op x x)
     have e105 := h (M.op y y) x (M.op y y)
     have e106 := h (M.op y y) y x
     have e107 := h (M.op y y) y y
     have e108 := h (M.op y y) y z
     have e109 := h (M.op y y) y (M.op x x)
     have e110 := h (M.op y y) y (M.op y y)
     have e111 := h (M.op y y) z x
     have e112 := h (M.op y y) z y
     have e113 := h (M.op y y) z z
     have e114 := h (M.op y y) z (M.op x x)
     have e115 := h (M.op y y) z (M.op y y)
     have e116 := h (M.op y y) (M.op x x) x
     have e117 := h (M.op y y) (M.op x x) y
     have e118 := h (M.op y y) (M.op x x) z
     have e119 := h (M.op y y) (M.op x x) (M.op x x)
     have e120 := h (M.op y y) (M.op x x) (M.op y y)
     have e121 := h (M.op y y) (M.op y y) x
     have e122 := h (M.op y y) (M.op y y) y
     have e123 := h (M.op y y) (M.op y y) z
     have e124 := h (M.op y y) (M.op y y) (M.op x x)
     have e125 := h (M.op y y) (M.op y y) (M.op y y)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q8424930_apply M K])

/-- `x = x ◇ (x ◇ x)` -/
theorem Equation8_StructuralFrom_Equation3480 : Law8.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7038766 M K ?_
  rw [@Law8.models_iff]
  intro x
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7038766_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7038766_dg M K, K.dsq]
     try simp only [q7038766_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7038766_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7038766_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x (M.op x x)
     have e3 := h x x (M.op x x) x
     have e4 := h x x (M.op x x) (M.op x x)
     have e5 := h x (M.op x x) x x
     have e6 := h x (M.op x x) x (M.op x x)
     have e7 := h x (M.op x x) (M.op x x) x
     have e8 := h x (M.op x x) (M.op x x) (M.op x x)
     have e9 := h (M.op x x) x x x
     have e10 := h (M.op x x) x x (M.op x x)
     have e11 := h (M.op x x) x (M.op x x) x
     have e12 := h (M.op x x) x (M.op x x) (M.op x x)
     have e13 := h (M.op x x) (M.op x x) x x
     have e14 := h (M.op x x) (M.op x x) x (M.op x x)
     have e15 := h (M.op x x) (M.op x x) (M.op x x) x
     have e16 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q7038766_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x (M.op x x)
     have e3 := h x x (M.op x x) x
     have e4 := h x x (M.op x x) (M.op x x)
     have e5 := h x (M.op x x) x x
     have e6 := h x (M.op x x) x (M.op x x)
     have e7 := h x (M.op x x) (M.op x x) x
     have e8 := h x (M.op x x) (M.op x x) (M.op x x)
     have e9 := h (M.op x x) x x x
     have e10 := h (M.op x x) x x (M.op x x)
     have e11 := h (M.op x x) x (M.op x x) x
     have e12 := h (M.op x x) x (M.op x x) (M.op x x)
     have e13 := h (M.op x x) (M.op x x) x x
     have e14 := h (M.op x x) (M.op x x) x (M.op x x)
     have e15 := h (M.op x x) (M.op x x) (M.op x x) x
     have e16 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7038766_apply M K])

/-- `x = x ◇ (y ◇ y)` -/
theorem Equation11_StructuralFrom_Equation3480 : Law11.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law11.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ (x ◇ (x ◇ (x ◇ x)))` -/
theorem Equation411_StructuralFrom_Equation3480 : Law411.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7038766 M K ?_
  rw [@Law411.models_iff]
  intro x
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7038766_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7038766_dg M K, K.dsq]
     try simp only [q7038766_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7038766_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7038766_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x (M.op x x)
     have e3 := h x x (M.op x x) x
     have e4 := h x x (M.op x x) (M.op x x)
     have e5 := h x (M.op x x) x x
     have e6 := h x (M.op x x) x (M.op x x)
     have e7 := h x (M.op x x) (M.op x x) x
     have e8 := h x (M.op x x) (M.op x x) (M.op x x)
     have e9 := h (M.op x x) x x x
     have e10 := h (M.op x x) x x (M.op x x)
     have e11 := h (M.op x x) x (M.op x x) x
     have e12 := h (M.op x x) x (M.op x x) (M.op x x)
     have e13 := h (M.op x x) (M.op x x) x x
     have e14 := h (M.op x x) (M.op x x) x (M.op x x)
     have e15 := h (M.op x x) (M.op x x) (M.op x x) x
     have e16 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q7038766_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x (M.op x x)
     have e3 := h x x (M.op x x) x
     have e4 := h x x (M.op x x) (M.op x x)
     have e5 := h x (M.op x x) x x
     have e6 := h x (M.op x x) x (M.op x x)
     have e7 := h x (M.op x x) (M.op x x) x
     have e8 := h x (M.op x x) (M.op x x) (M.op x x)
     have e9 := h (M.op x x) x x x
     have e10 := h (M.op x x) x x (M.op x x)
     have e11 := h (M.op x x) x (M.op x x) x
     have e12 := h (M.op x x) x (M.op x x) (M.op x x)
     have e13 := h (M.op x x) (M.op x x) x x
     have e14 := h (M.op x x) (M.op x x) x (M.op x x)
     have e15 := h (M.op x x) (M.op x x) (M.op x x) x
     have e16 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7038766_apply M K])

/-- `x = x ◇ (x ◇ (x ◇ (y ◇ y)))` -/
theorem Equation414_StructuralFrom_Equation3480 : Law414.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law414.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ (y ◇ (y ◇ (x ◇ x)))` -/
theorem Equation436_StructuralFrom_Equation3480 : Law436.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law436.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ (y ◇ (y ◇ (y ◇ y)))` -/
theorem Equation440_StructuralFrom_Equation3480 : Law440.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law440.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ ((x ◇ x) ◇ (x ◇ x))` -/
theorem Equation817_StructuralFrom_Equation3480 : Law817.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law817.models_iff]
  intro x
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x (M.op x x)
     have e3 := h x x (M.op x x) x
     have e4 := h x x (M.op x x) (M.op x x)
     have e5 := h x (M.op x x) x x
     have e6 := h x (M.op x x) x (M.op x x)
     have e7 := h x (M.op x x) (M.op x x) x
     have e8 := h x (M.op x x) (M.op x x) (M.op x x)
     have e9 := h (M.op x x) x x x
     have e10 := h (M.op x x) x x (M.op x x)
     have e11 := h (M.op x x) x (M.op x x) x
     have e12 := h (M.op x x) x (M.op x x) (M.op x x)
     have e13 := h (M.op x x) (M.op x x) x x
     have e14 := h (M.op x x) (M.op x x) x (M.op x x)
     have e15 := h (M.op x x) (M.op x x) (M.op x x) x
     have e16 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x (M.op x x)
     have e3 := h x x (M.op x x) x
     have e4 := h x x (M.op x x) (M.op x x)
     have e5 := h x (M.op x x) x x
     have e6 := h x (M.op x x) x (M.op x x)
     have e7 := h x (M.op x x) (M.op x x) x
     have e8 := h x (M.op x x) (M.op x x) (M.op x x)
     have e9 := h (M.op x x) x x x
     have e10 := h (M.op x x) x x (M.op x x)
     have e11 := h (M.op x x) x (M.op x x) x
     have e12 := h (M.op x x) x (M.op x x) (M.op x x)
     have e13 := h (M.op x x) (M.op x x) x x
     have e14 := h (M.op x x) (M.op x x) x (M.op x x)
     have e15 := h (M.op x x) (M.op x x) (M.op x x) x
     have e16 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ ((x ◇ x) ◇ (y ◇ y))` -/
theorem Equation820_StructuralFrom_Equation3480 : Law820.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law820.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ ((x ◇ y) ◇ (x ◇ y))` -/
theorem Equation823_StructuralFrom_Equation3480 : Law823.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law823.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ ((y ◇ x) ◇ (y ◇ x))` -/
theorem Equation835_StructuralFrom_Equation3480 : Law835.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law835.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ ((y ◇ y) ◇ (x ◇ x))` -/
theorem Equation842_StructuralFrom_Equation3480 : Law842.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law842.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ ((y ◇ y) ◇ (y ◇ y))` -/
theorem Equation846_StructuralFrom_Equation3480 : Law846.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law846.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ ((x ◇ (x ◇ x)) ◇ x)` -/
theorem Equation1020_StructuralFrom_Equation3480 : Law1020.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7038766 M K ?_
  rw [@Law1020.models_iff]
  intro x
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7038766_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7038766_dg M K, K.dsq]
     try simp only [q7038766_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7038766_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7038766_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x (M.op x x)
     have e3 := h x x (M.op x x) x
     have e4 := h x x (M.op x x) (M.op x x)
     have e5 := h x (M.op x x) x x
     have e6 := h x (M.op x x) x (M.op x x)
     have e7 := h x (M.op x x) (M.op x x) x
     have e8 := h x (M.op x x) (M.op x x) (M.op x x)
     have e9 := h (M.op x x) x x x
     have e10 := h (M.op x x) x x (M.op x x)
     have e11 := h (M.op x x) x (M.op x x) x
     have e12 := h (M.op x x) x (M.op x x) (M.op x x)
     have e13 := h (M.op x x) (M.op x x) x x
     have e14 := h (M.op x x) (M.op x x) x (M.op x x)
     have e15 := h (M.op x x) (M.op x x) (M.op x x) x
     have e16 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q7038766_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x (M.op x x)
     have e3 := h x x (M.op x x) x
     have e4 := h x x (M.op x x) (M.op x x)
     have e5 := h x (M.op x x) x x
     have e6 := h x (M.op x x) x (M.op x x)
     have e7 := h x (M.op x x) (M.op x x) x
     have e8 := h x (M.op x x) (M.op x x) (M.op x x)
     have e9 := h (M.op x x) x x x
     have e10 := h (M.op x x) x x (M.op x x)
     have e11 := h (M.op x x) x (M.op x x) x
     have e12 := h (M.op x x) x (M.op x x) (M.op x x)
     have e13 := h (M.op x x) (M.op x x) x x
     have e14 := h (M.op x x) (M.op x x) x (M.op x x)
     have e15 := h (M.op x x) (M.op x x) (M.op x x) x
     have e16 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7038766_apply M K])

/-- `x = x ◇ ((x ◇ (y ◇ y)) ◇ x)` -/
theorem Equation1028_StructuralFrom_Equation3480 : Law1028.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7038766 M K ?_
  rw [@Law1028.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7038766_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7038766_dg M K, K.dsq]
     try simp only [q7038766_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7038766_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7038766_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q7038766_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7038766_apply M K])

/-- `x = x ◇ ((y ◇ (x ◇ x)) ◇ y)` -/
theorem Equation1036_StructuralFrom_Equation3480 : Law1036.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law1036.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ ((y ◇ (y ◇ y)) ◇ y)` -/
theorem Equation1049_StructuralFrom_Equation3480 : Law1049.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law1049.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ (((x ◇ x) ◇ x) ◇ x)` -/
theorem Equation1223_StructuralFrom_Equation3480 : Law1223.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law1223.models_iff]
  intro x
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x (M.op x x)
     have e3 := h x x (M.op x x) x
     have e4 := h x x (M.op x x) (M.op x x)
     have e5 := h x (M.op x x) x x
     have e6 := h x (M.op x x) x (M.op x x)
     have e7 := h x (M.op x x) (M.op x x) x
     have e8 := h x (M.op x x) (M.op x x) (M.op x x)
     have e9 := h (M.op x x) x x x
     have e10 := h (M.op x x) x x (M.op x x)
     have e11 := h (M.op x x) x (M.op x x) x
     have e12 := h (M.op x x) x (M.op x x) (M.op x x)
     have e13 := h (M.op x x) (M.op x x) x x
     have e14 := h (M.op x x) (M.op x x) x (M.op x x)
     have e15 := h (M.op x x) (M.op x x) (M.op x x) x
     have e16 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x (M.op x x)
     have e3 := h x x (M.op x x) x
     have e4 := h x x (M.op x x) (M.op x x)
     have e5 := h x (M.op x x) x x
     have e6 := h x (M.op x x) x (M.op x x)
     have e7 := h x (M.op x x) (M.op x x) x
     have e8 := h x (M.op x x) (M.op x x) (M.op x x)
     have e9 := h (M.op x x) x x x
     have e10 := h (M.op x x) x x (M.op x x)
     have e11 := h (M.op x x) x (M.op x x) x
     have e12 := h (M.op x x) x (M.op x x) (M.op x x)
     have e13 := h (M.op x x) (M.op x x) x x
     have e14 := h (M.op x x) (M.op x x) x (M.op x x)
     have e15 := h (M.op x x) (M.op x x) (M.op x x) x
     have e16 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ (((x ◇ x) ◇ y) ◇ y)` -/
theorem Equation1226_StructuralFrom_Equation3480 : Law1226.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law1226.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ (((y ◇ y) ◇ x) ◇ x)` -/
theorem Equation1248_StructuralFrom_Equation3480 : Law1248.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law1248.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ (((y ◇ y) ◇ y) ◇ y)` -/
theorem Equation1252_StructuralFrom_Equation3480 : Law1252.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law1252.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ (((y ◇ y) ◇ z) ◇ z)` -/
theorem Equation1256_StructuralFrom_Equation3480 : Law1256.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law1256.models_iff]
  intro x y z
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x z
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y z
     have e7 := h x x z x
     have e8 := h x x z y
     have e9 := h x x z z
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x z
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y z
     have e16 := h x y z x
     have e17 := h x y z y
     have e18 := h x y z z
     have e19 := h x z x x
     have e20 := h x z x y
     have e21 := h x z x z
     have e22 := h x z y x
     have e23 := h x z y y
     have e24 := h x z y z
     have e25 := h x z z x
     have e26 := h x z z y
     have e27 := h x z z z
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x z
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y z
     have e34 := h y x z x
     have e35 := h y x z y
     have e36 := h y x z z
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x z
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y z
     have e43 := h y y z x
     have e44 := h y y z y
     have e45 := h y y z z
     have e46 := h y z x x
     have e47 := h y z x y
     have e48 := h y z x z
     have e49 := h y z y x
     have e50 := h y z y y
     have e51 := h y z y z
     have e52 := h y z z x
     have e53 := h y z z y
     have e54 := h y z z z
     have e55 := h z x x x
     have e56 := h z x x y
     have e57 := h z x x z
     have e58 := h z x y x
     have e59 := h z x y y
     have e60 := h z x y z
     have e61 := h z x z x
     have e62 := h z x z y
     have e63 := h z x z z
     have e64 := h z y x x
     have e65 := h z y x y
     have e66 := h z y x z
     have e67 := h z y y x
     have e68 := h z y y y
     have e69 := h z y y z
     have e70 := h z y z x
     have e71 := h z y z y
     have e72 := h z y z z
     have e73 := h z z x x
     have e74 := h z z x y
     have e75 := h z z x z
     have e76 := h z z y x
     have e77 := h z z y y
     have e78 := h z z y z
     have e79 := h z z z x
     have e80 := h z z z y
     have e81 := h z z z z
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x z
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y z
     have e7 := h x x z x
     have e8 := h x x z y
     have e9 := h x x z z
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x z
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y z
     have e16 := h x y z x
     have e17 := h x y z y
     have e18 := h x y z z
     have e19 := h x z x x
     have e20 := h x z x y
     have e21 := h x z x z
     have e22 := h x z y x
     have e23 := h x z y y
     have e24 := h x z y z
     have e25 := h x z z x
     have e26 := h x z z y
     have e27 := h x z z z
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x z
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y z
     have e34 := h y x z x
     have e35 := h y x z y
     have e36 := h y x z z
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x z
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y z
     have e43 := h y y z x
     have e44 := h y y z y
     have e45 := h y y z z
     have e46 := h y z x x
     have e47 := h y z x y
     have e48 := h y z x z
     have e49 := h y z y x
     have e50 := h y z y y
     have e51 := h y z y z
     have e52 := h y z z x
     have e53 := h y z z y
     have e54 := h y z z z
     have e55 := h z x x x
     have e56 := h z x x y
     have e57 := h z x x z
     have e58 := h z x y x
     have e59 := h z x y y
     have e60 := h z x y z
     have e61 := h z x z x
     have e62 := h z x z y
     have e63 := h z x z z
     have e64 := h z y x x
     have e65 := h z y x y
     have e66 := h z y x z
     have e67 := h z y y x
     have e68 := h z y y y
     have e69 := h z y y z
     have e70 := h z y z x
     have e71 := h z y z y
     have e72 := h z y z z
     have e73 := h z z x x
     have e74 := h z z x y
     have e75 := h z z x z
     have e76 := h z z y x
     have e77 := h z z y y
     have e78 := h z z y z
     have e79 := h z z z x
     have e80 := h z z z y
     have e81 := h z z z z
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = (x ◇ x) ◇ ((x ◇ x) ◇ x)` -/
theorem Equation1629_StructuralFrom_Equation3480 : Law1629.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q5126831 M K ?_
  rw [@Law1629.models_iff]
  intro x
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q5126831_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q5126831_dg M K, K.dsq]
     try simp only [q5126831_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q5126831_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q5126831_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x (M.op x x)
     have e3 := h x x (M.op x x) x
     have e4 := h x x (M.op x x) (M.op x x)
     have e5 := h x (M.op x x) x x
     have e6 := h x (M.op x x) x (M.op x x)
     have e7 := h x (M.op x x) (M.op x x) x
     have e8 := h x (M.op x x) (M.op x x) (M.op x x)
     have e9 := h (M.op x x) x x x
     have e10 := h (M.op x x) x x (M.op x x)
     have e11 := h (M.op x x) x (M.op x x) x
     have e12 := h (M.op x x) x (M.op x x) (M.op x x)
     have e13 := h (M.op x x) (M.op x x) x x
     have e14 := h (M.op x x) (M.op x x) x (M.op x x)
     have e15 := h (M.op x x) (M.op x x) (M.op x x) x
     have e16 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q5126831_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x (M.op x x)
     have e3 := h x x (M.op x x) x
     have e4 := h x x (M.op x x) (M.op x x)
     have e5 := h x (M.op x x) x x
     have e6 := h x (M.op x x) x (M.op x x)
     have e7 := h x (M.op x x) (M.op x x) x
     have e8 := h x (M.op x x) (M.op x x) (M.op x x)
     have e9 := h (M.op x x) x x x
     have e10 := h (M.op x x) x x (M.op x x)
     have e11 := h (M.op x x) x (M.op x x) x
     have e12 := h (M.op x x) x (M.op x x) (M.op x x)
     have e13 := h (M.op x x) (M.op x x) x x
     have e14 := h (M.op x x) (M.op x x) x (M.op x x)
     have e15 := h (M.op x x) (M.op x x) (M.op x x) x
     have e16 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q5126831_apply M K])

/-- `x = (x ◇ x) ◇ ((y ◇ y) ◇ x)` -/
theorem Equation1637_StructuralFrom_Equation3480 : Law1637.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q5126831 M K ?_
  rw [@Law1637.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q5126831_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q5126831_dg M K, K.dsq]
     try simp only [q5126831_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q5126831_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q5126831_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q5126831_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q5126831_apply M K])

/-- `x = (y ◇ y) ◇ ((x ◇ x) ◇ x)` -/
theorem Equation1718_StructuralFrom_Equation3480 : Law1718.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q5132319 M K ?_
  rw [@Law1718.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q5132319_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q5132319_dg M K, K.dsq]
     try simp only [q5132319_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q5132319_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q5132319_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q5132319_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q5132319_apply M K])

/-- `x = (y ◇ y) ◇ ((y ◇ y) ◇ x)` -/
theorem Equation1731_StructuralFrom_Equation3480 : Law1731.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q5132319 M K ?_
  rw [@Law1731.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q5132319_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q5132319_dg M K, K.dsq]
     try simp only [q5132319_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q5132319_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q5132319_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q5132319_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q5132319_apply M K])

/-- `x ◇ y = x ◇ (y ◇ (x ◇ x))` -/
theorem Equation3315_StructuralFrom_Equation3480 : Law3315.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7052094 M K ?_
  rw [@Law3315.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7052094_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7052094_dg M K, K.dsq]
     try simp only [q7052094_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7052094_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7052094_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q7052094_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7052094_apply M K])

/-- `x ◇ y = x ◇ (y ◇ (y ◇ y))` -/
theorem Equation3319_StructuralFrom_Equation3480 : Law3319.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7038766 M K ?_
  rw [@Law3319.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7038766_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7038766_dg M K, K.dsq]
     try simp only [q7038766_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7038766_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7038766_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q7038766_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7038766_apply M K])

/-- `x ◇ y = x ◇ (y ◇ (z ◇ z))` -/
theorem Equation3323_StructuralFrom_Equation3480 : Law3323.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7052094 M K ?_
  rw [@Law3323.models_iff]
  intro x y z
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7052094_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7052094_dg M K, K.dsq]
     try simp only [q7052094_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7052094_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7052094_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x z
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y z
     have e7 := h x x z x
     have e8 := h x x z y
     have e9 := h x x z z
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x z
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y z
     have e16 := h x y z x
     have e17 := h x y z y
     have e18 := h x y z z
     have e19 := h x z x x
     have e20 := h x z x y
     have e21 := h x z x z
     have e22 := h x z y x
     have e23 := h x z y y
     have e24 := h x z y z
     have e25 := h x z z x
     have e26 := h x z z y
     have e27 := h x z z z
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x z
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y z
     have e34 := h y x z x
     have e35 := h y x z y
     have e36 := h y x z z
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x z
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y z
     have e43 := h y y z x
     have e44 := h y y z y
     have e45 := h y y z z
     have e46 := h y z x x
     have e47 := h y z x y
     have e48 := h y z x z
     have e49 := h y z y x
     have e50 := h y z y y
     have e51 := h y z y z
     have e52 := h y z z x
     have e53 := h y z z y
     have e54 := h y z z z
     have e55 := h z x x x
     have e56 := h z x x y
     have e57 := h z x x z
     have e58 := h z x y x
     have e59 := h z x y y
     have e60 := h z x y z
     have e61 := h z x z x
     have e62 := h z x z y
     have e63 := h z x z z
     have e64 := h z y x x
     have e65 := h z y x y
     have e66 := h z y x z
     have e67 := h z y y x
     have e68 := h z y y y
     have e69 := h z y y z
     have e70 := h z y z x
     have e71 := h z y z y
     have e72 := h z y z z
     have e73 := h z z x x
     have e74 := h z z x y
     have e75 := h z z x z
     have e76 := h z z y x
     have e77 := h z z y y
     have e78 := h z z y z
     have e79 := h z z z x
     have e80 := h z z z y
     have e81 := h z z z z
     grind [q7052094_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x z
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y z
     have e7 := h x x z x
     have e8 := h x x z y
     have e9 := h x x z z
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x z
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y z
     have e16 := h x y z x
     have e17 := h x y z y
     have e18 := h x y z z
     have e19 := h x z x x
     have e20 := h x z x y
     have e21 := h x z x z
     have e22 := h x z y x
     have e23 := h x z y y
     have e24 := h x z y z
     have e25 := h x z z x
     have e26 := h x z z y
     have e27 := h x z z z
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x z
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y z
     have e34 := h y x z x
     have e35 := h y x z y
     have e36 := h y x z z
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x z
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y z
     have e43 := h y y z x
     have e44 := h y y z y
     have e45 := h y y z z
     have e46 := h y z x x
     have e47 := h y z x y
     have e48 := h y z x z
     have e49 := h y z y x
     have e50 := h y z y y
     have e51 := h y z y z
     have e52 := h y z z x
     have e53 := h y z z y
     have e54 := h y z z z
     have e55 := h z x x x
     have e56 := h z x x y
     have e57 := h z x x z
     have e58 := h z x y x
     have e59 := h z x y y
     have e60 := h z x y z
     have e61 := h z x z x
     have e62 := h z x z y
     have e63 := h z x z z
     have e64 := h z y x x
     have e65 := h z y x y
     have e66 := h z y x z
     have e67 := h z y y x
     have e68 := h z y y y
     have e69 := h z y y z
     have e70 := h z y z x
     have e71 := h z y z y
     have e72 := h z y z z
     have e73 := h z z x x
     have e74 := h z z x y
     have e75 := h z z x z
     have e76 := h z z y x
     have e77 := h z z y y
     have e78 := h z z y z
     have e79 := h z z z x
     have e80 := h z z z y
     have e81 := h z z z z
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7052094_apply M K])

/-- `x ◇ y = x ◇ ((x ◇ x) ◇ y)` -/
theorem Equation3509_StructuralFrom_Equation3480 : Law3509.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q5132319 M K ?_
  rw [@Law3509.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q5132319_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q5132319_dg M K, K.dsq]
     try simp only [q5132319_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q5132319_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q5132319_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q5132319_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q5132319_apply M K])

/-- `x ◇ y = x ◇ ((y ◇ y) ◇ y)` -/
theorem Equation3522_StructuralFrom_Equation3480 : Law3522.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q5126831 M K ?_
  rw [@Law3522.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q5126831_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q5126831_dg M K, K.dsq]
     try simp only [q5126831_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q5126831_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q5126831_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q5126831_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q5126831_apply M K])

/-- `x ◇ y = x ◇ ((z ◇ z) ◇ y)` -/
theorem Equation3537_StructuralFrom_Equation3480 : Law3537.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q5132319 M K ?_
  rw [@Law3537.models_iff]
  intro x y z
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q5132319_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q5132319_dg M K, K.dsq]
     try simp only [q5132319_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q5132319_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q5132319_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x z
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y z
     have e7 := h x x z x
     have e8 := h x x z y
     have e9 := h x x z z
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x z
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y z
     have e16 := h x y z x
     have e17 := h x y z y
     have e18 := h x y z z
     have e19 := h x z x x
     have e20 := h x z x y
     have e21 := h x z x z
     have e22 := h x z y x
     have e23 := h x z y y
     have e24 := h x z y z
     have e25 := h x z z x
     have e26 := h x z z y
     have e27 := h x z z z
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x z
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y z
     have e34 := h y x z x
     have e35 := h y x z y
     have e36 := h y x z z
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x z
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y z
     have e43 := h y y z x
     have e44 := h y y z y
     have e45 := h y y z z
     have e46 := h y z x x
     have e47 := h y z x y
     have e48 := h y z x z
     have e49 := h y z y x
     have e50 := h y z y y
     have e51 := h y z y z
     have e52 := h y z z x
     have e53 := h y z z y
     have e54 := h y z z z
     have e55 := h z x x x
     have e56 := h z x x y
     have e57 := h z x x z
     have e58 := h z x y x
     have e59 := h z x y y
     have e60 := h z x y z
     have e61 := h z x z x
     have e62 := h z x z y
     have e63 := h z x z z
     have e64 := h z y x x
     have e65 := h z y x y
     have e66 := h z y x z
     have e67 := h z y y x
     have e68 := h z y y y
     have e69 := h z y y z
     have e70 := h z y z x
     have e71 := h z y z y
     have e72 := h z y z z
     have e73 := h z z x x
     have e74 := h z z x y
     have e75 := h z z x z
     have e76 := h z z y x
     have e77 := h z z y y
     have e78 := h z z y z
     have e79 := h z z z x
     have e80 := h z z z y
     have e81 := h z z z z
     grind [q5132319_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x z
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y z
     have e7 := h x x z x
     have e8 := h x x z y
     have e9 := h x x z z
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x z
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y z
     have e16 := h x y z x
     have e17 := h x y z y
     have e18 := h x y z z
     have e19 := h x z x x
     have e20 := h x z x y
     have e21 := h x z x z
     have e22 := h x z y x
     have e23 := h x z y y
     have e24 := h x z y z
     have e25 := h x z z x
     have e26 := h x z z y
     have e27 := h x z z z
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x z
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y z
     have e34 := h y x z x
     have e35 := h y x z y
     have e36 := h y x z z
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x z
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y z
     have e43 := h y y z x
     have e44 := h y y z y
     have e45 := h y y z z
     have e46 := h y z x x
     have e47 := h y z x y
     have e48 := h y z x z
     have e49 := h y z y x
     have e50 := h y z y y
     have e51 := h y z y z
     have e52 := h y z z x
     have e53 := h y z z y
     have e54 := h y z z z
     have e55 := h z x x x
     have e56 := h z x x y
     have e57 := h z x x z
     have e58 := h z x y x
     have e59 := h z x y y
     have e60 := h z x y z
     have e61 := h z x z x
     have e62 := h z x z y
     have e63 := h z x z z
     have e64 := h z y x x
     have e65 := h z y x y
     have e66 := h z y x z
     have e67 := h z y y x
     have e68 := h z y y y
     have e69 := h z y y z
     have e70 := h z y z x
     have e71 := h z y z y
     have e72 := h z y z z
     have e73 := h z z x x
     have e74 := h z z x y
     have e75 := h z z x z
     have e76 := h z z y x
     have e77 := h z z y y
     have e78 := h z z y z
     have e79 := h z z z x
     have e80 := h z z z y
     have e81 := h z z z z
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q5132319_apply M K])

/-- `x ◇ y = (x ◇ x) ◇ (x ◇ y)` -/
theorem Equation3712_StructuralFrom_Equation3480 : Law3712.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q5132319 M K ?_
  rw [@Law3712.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q5132319_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q5132319_dg M K, K.dsq]
     try simp only [q5132319_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q5132319_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q5132319_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q5132319_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q5132319_apply M K])

/-- `x ◇ y = (x ◇ y) ◇ (x ◇ x)` -/
theorem Equation3721_StructuralFrom_Equation3480 : Law3721.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law3721.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x ◇ y = (x ◇ y) ◇ (z ◇ z)` -/
theorem Equation3729_StructuralFrom_Equation3480 : Law3729.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law3729.models_iff]
  intro x y z
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x z
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y z
     have e7 := h x x z x
     have e8 := h x x z y
     have e9 := h x x z z
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x z
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y z
     have e16 := h x y z x
     have e17 := h x y z y
     have e18 := h x y z z
     have e19 := h x z x x
     have e20 := h x z x y
     have e21 := h x z x z
     have e22 := h x z y x
     have e23 := h x z y y
     have e24 := h x z y z
     have e25 := h x z z x
     have e26 := h x z z y
     have e27 := h x z z z
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x z
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y z
     have e34 := h y x z x
     have e35 := h y x z y
     have e36 := h y x z z
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x z
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y z
     have e43 := h y y z x
     have e44 := h y y z y
     have e45 := h y y z z
     have e46 := h y z x x
     have e47 := h y z x y
     have e48 := h y z x z
     have e49 := h y z y x
     have e50 := h y z y y
     have e51 := h y z y z
     have e52 := h y z z x
     have e53 := h y z z y
     have e54 := h y z z z
     have e55 := h z x x x
     have e56 := h z x x y
     have e57 := h z x x z
     have e58 := h z x y x
     have e59 := h z x y y
     have e60 := h z x y z
     have e61 := h z x z x
     have e62 := h z x z y
     have e63 := h z x z z
     have e64 := h z y x x
     have e65 := h z y x y
     have e66 := h z y x z
     have e67 := h z y y x
     have e68 := h z y y y
     have e69 := h z y y z
     have e70 := h z y z x
     have e71 := h z y z y
     have e72 := h z y z z
     have e73 := h z z x x
     have e74 := h z z x y
     have e75 := h z z x z
     have e76 := h z z y x
     have e77 := h z z y y
     have e78 := h z z y z
     have e79 := h z z z x
     have e80 := h z z z y
     have e81 := h z z z z
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x z
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y z
     have e7 := h x x z x
     have e8 := h x x z y
     have e9 := h x x z z
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x z
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y z
     have e16 := h x y z x
     have e17 := h x y z y
     have e18 := h x y z z
     have e19 := h x z x x
     have e20 := h x z x y
     have e21 := h x z x z
     have e22 := h x z y x
     have e23 := h x z y y
     have e24 := h x z y z
     have e25 := h x z z x
     have e26 := h x z z y
     have e27 := h x z z z
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x z
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y z
     have e34 := h y x z x
     have e35 := h y x z y
     have e36 := h y x z z
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x z
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y z
     have e43 := h y y z x
     have e44 := h y y z y
     have e45 := h y y z z
     have e46 := h y z x x
     have e47 := h y z x y
     have e48 := h y z x z
     have e49 := h y z y x
     have e50 := h y z y y
     have e51 := h y z y z
     have e52 := h y z z x
     have e53 := h y z z y
     have e54 := h y z z z
     have e55 := h z x x x
     have e56 := h z x x y
     have e57 := h z x x z
     have e58 := h z x y x
     have e59 := h z x y y
     have e60 := h z x y z
     have e61 := h z x z x
     have e62 := h z x z y
     have e63 := h z x z z
     have e64 := h z y x x
     have e65 := h z y x y
     have e66 := h z y x z
     have e67 := h z y y x
     have e68 := h z y y y
     have e69 := h z y y z
     have e70 := h z y z x
     have e71 := h z y z y
     have e72 := h z y z z
     have e73 := h z z x x
     have e74 := h z z x y
     have e75 := h z z x z
     have e76 := h z z y x
     have e77 := h z z y y
     have e78 := h z z y z
     have e79 := h z z z x
     have e80 := h z z z y
     have e81 := h z z z z
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x ◇ (x ◇ x) = (x ◇ x) ◇ x` -/
theorem Equation4380_StructuralFrom_Equation3480 : Law4380.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q8424930 M K ?_
  rw [@Law4380.models_iff]
  intro x
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q8424930_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q8424930_dg M K, K.dsq]
     try simp only [q8424930_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q8424930_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q8424930_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x (M.op x x)
     have e3 := h x x (M.op x x) x
     have e4 := h x x (M.op x x) (M.op x x)
     have e5 := h x (M.op x x) x x
     have e6 := h x (M.op x x) x (M.op x x)
     have e7 := h x (M.op x x) (M.op x x) x
     have e8 := h x (M.op x x) (M.op x x) (M.op x x)
     have e9 := h (M.op x x) x x x
     have e10 := h (M.op x x) x x (M.op x x)
     have e11 := h (M.op x x) x (M.op x x) x
     have e12 := h (M.op x x) x (M.op x x) (M.op x x)
     have e13 := h (M.op x x) (M.op x x) x x
     have e14 := h (M.op x x) (M.op x x) x (M.op x x)
     have e15 := h (M.op x x) (M.op x x) (M.op x x) x
     have e16 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q8424930_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x (M.op x x)
     have e3 := h x x (M.op x x) x
     have e4 := h x x (M.op x x) (M.op x x)
     have e5 := h x (M.op x x) x x
     have e6 := h x (M.op x x) x (M.op x x)
     have e7 := h x (M.op x x) (M.op x x) x
     have e8 := h x (M.op x x) (M.op x x) (M.op x x)
     have e9 := h (M.op x x) x x x
     have e10 := h (M.op x x) x x (M.op x x)
     have e11 := h (M.op x x) x (M.op x x) x
     have e12 := h (M.op x x) x (M.op x x) (M.op x x)
     have e13 := h (M.op x x) (M.op x x) x x
     have e14 := h (M.op x x) (M.op x x) x (M.op x x)
     have e15 := h (M.op x x) (M.op x x) (M.op x x) x
     have e16 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q8424930_apply M K])

/-- `x ◇ (x ◇ x) = (y ◇ y) ◇ x` -/
theorem Equation4388_StructuralFrom_Equation3480 : Law4388.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q8424930 M K ?_
  rw [@Law4388.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q8424930_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q8424930_dg M K, K.dsq]
     try simp only [q8424930_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q8424930_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q8424930_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q8424930_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q8424930_apply M K])

/-- `x ◇ (y ◇ y) = (y ◇ y) ◇ x` -/
theorem Equation4482_StructuralFrom_Equation3480 : Law4482.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q8424930 M K ?_
  rw [@Law4482.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q8424930_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q8424930_dg M K, K.dsq]
     try simp only [q8424930_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q8424930_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q8424930_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind [q8424930_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x (M.op x x)
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y (M.op x x)
     have e7 := h x x (M.op x x) x
     have e8 := h x x (M.op x x) y
     have e9 := h x x (M.op x x) (M.op x x)
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x (M.op x x)
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y (M.op x x)
     have e16 := h x y (M.op x x) x
     have e17 := h x y (M.op x x) y
     have e18 := h x y (M.op x x) (M.op x x)
     have e19 := h x (M.op x x) x x
     have e20 := h x (M.op x x) x y
     have e21 := h x (M.op x x) x (M.op x x)
     have e22 := h x (M.op x x) y x
     have e23 := h x (M.op x x) y y
     have e24 := h x (M.op x x) y (M.op x x)
     have e25 := h x (M.op x x) (M.op x x) x
     have e26 := h x (M.op x x) (M.op x x) y
     have e27 := h x (M.op x x) (M.op x x) (M.op x x)
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x (M.op x x)
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y (M.op x x)
     have e34 := h y x (M.op x x) x
     have e35 := h y x (M.op x x) y
     have e36 := h y x (M.op x x) (M.op x x)
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x (M.op x x)
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y (M.op x x)
     have e43 := h y y (M.op x x) x
     have e44 := h y y (M.op x x) y
     have e45 := h y y (M.op x x) (M.op x x)
     have e46 := h y (M.op x x) x x
     have e47 := h y (M.op x x) x y
     have e48 := h y (M.op x x) x (M.op x x)
     have e49 := h y (M.op x x) y x
     have e50 := h y (M.op x x) y y
     have e51 := h y (M.op x x) y (M.op x x)
     have e52 := h y (M.op x x) (M.op x x) x
     have e53 := h y (M.op x x) (M.op x x) y
     have e54 := h y (M.op x x) (M.op x x) (M.op x x)
     have e55 := h (M.op x x) x x x
     have e56 := h (M.op x x) x x y
     have e57 := h (M.op x x) x x (M.op x x)
     have e58 := h (M.op x x) x y x
     have e59 := h (M.op x x) x y y
     have e60 := h (M.op x x) x y (M.op x x)
     have e61 := h (M.op x x) x (M.op x x) x
     have e62 := h (M.op x x) x (M.op x x) y
     have e63 := h (M.op x x) x (M.op x x) (M.op x x)
     have e64 := h (M.op x x) y x x
     have e65 := h (M.op x x) y x y
     have e66 := h (M.op x x) y x (M.op x x)
     have e67 := h (M.op x x) y y x
     have e68 := h (M.op x x) y y y
     have e69 := h (M.op x x) y y (M.op x x)
     have e70 := h (M.op x x) y (M.op x x) x
     have e71 := h (M.op x x) y (M.op x x) y
     have e72 := h (M.op x x) y (M.op x x) (M.op x x)
     have e73 := h (M.op x x) (M.op x x) x x
     have e74 := h (M.op x x) (M.op x x) x y
     have e75 := h (M.op x x) (M.op x x) x (M.op x x)
     have e76 := h (M.op x x) (M.op x x) y x
     have e77 := h (M.op x x) (M.op x x) y y
     have e78 := h (M.op x x) (M.op x x) y (M.op x x)
     have e79 := h (M.op x x) (M.op x x) (M.op x x) x
     have e80 := h (M.op x x) (M.op x x) (M.op x x) y
     have e81 := h (M.op x x) (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q8424930_apply M K])

/-- `x ◇ (y ◇ y) = (z ◇ z) ◇ x` -/
theorem Equation4497_StructuralFrom_Equation3480 : Law4497.StructuralFrom Law3480 := by
  intro G M hM
  have h : Equation3480 G := Law3480.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3480 h)⟩
  refine structural_q8424930 M K ?_
  rw [@Law4497.models_iff]
  intro x y z
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind [q8424930_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q8424930_dg M K, K.dsq]
     try simp only [q8424930_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q8424930_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q8424930_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x z
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y z
     have e7 := h x x z x
     have e8 := h x x z y
     have e9 := h x x z z
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x z
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y z
     have e16 := h x y z x
     have e17 := h x y z y
     have e18 := h x y z z
     have e19 := h x z x x
     have e20 := h x z x y
     have e21 := h x z x z
     have e22 := h x z y x
     have e23 := h x z y y
     have e24 := h x z y z
     have e25 := h x z z x
     have e26 := h x z z y
     have e27 := h x z z z
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x z
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y z
     have e34 := h y x z x
     have e35 := h y x z y
     have e36 := h y x z z
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x z
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y z
     have e43 := h y y z x
     have e44 := h y y z y
     have e45 := h y y z z
     have e46 := h y z x x
     have e47 := h y z x y
     have e48 := h y z x z
     have e49 := h y z y x
     have e50 := h y z y y
     have e51 := h y z y z
     have e52 := h y z z x
     have e53 := h y z z y
     have e54 := h y z z z
     have e55 := h z x x x
     have e56 := h z x x y
     have e57 := h z x x z
     have e58 := h z x y x
     have e59 := h z x y y
     have e60 := h z x y z
     have e61 := h z x z x
     have e62 := h z x z y
     have e63 := h z x z z
     have e64 := h z y x x
     have e65 := h z y x y
     have e66 := h z y x z
     have e67 := h z y y x
     have e68 := h z y y y
     have e69 := h z y y z
     have e70 := h z y z x
     have e71 := h z y z y
     have e72 := h z y z z
     have e73 := h z z x x
     have e74 := h z z x y
     have e75 := h z z x z
     have e76 := h z z y x
     have e77 := h z z y y
     have e78 := h z z y z
     have e79 := h z z z x
     have e80 := h z z z y
     have e81 := h z z z z
     grind [q8424930_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3480_sqeq h K
     have g2 := f3480_sqidem h K
     have g3 := f3480_sqfixl h K
     have g4 := f3480_sqsqeq h K
     have e1 := h x x x x
     have e2 := h x x x y
     have e3 := h x x x z
     have e4 := h x x y x
     have e5 := h x x y y
     have e6 := h x x y z
     have e7 := h x x z x
     have e8 := h x x z y
     have e9 := h x x z z
     have e10 := h x y x x
     have e11 := h x y x y
     have e12 := h x y x z
     have e13 := h x y y x
     have e14 := h x y y y
     have e15 := h x y y z
     have e16 := h x y z x
     have e17 := h x y z y
     have e18 := h x y z z
     have e19 := h x z x x
     have e20 := h x z x y
     have e21 := h x z x z
     have e22 := h x z y x
     have e23 := h x z y y
     have e24 := h x z y z
     have e25 := h x z z x
     have e26 := h x z z y
     have e27 := h x z z z
     have e28 := h y x x x
     have e29 := h y x x y
     have e30 := h y x x z
     have e31 := h y x y x
     have e32 := h y x y y
     have e33 := h y x y z
     have e34 := h y x z x
     have e35 := h y x z y
     have e36 := h y x z z
     have e37 := h y y x x
     have e38 := h y y x y
     have e39 := h y y x z
     have e40 := h y y y x
     have e41 := h y y y y
     have e42 := h y y y z
     have e43 := h y y z x
     have e44 := h y y z y
     have e45 := h y y z z
     have e46 := h y z x x
     have e47 := h y z x y
     have e48 := h y z x z
     have e49 := h y z y x
     have e50 := h y z y y
     have e51 := h y z y z
     have e52 := h y z z x
     have e53 := h y z z y
     have e54 := h y z z z
     have e55 := h z x x x
     have e56 := h z x x y
     have e57 := h z x x z
     have e58 := h z x y x
     have e59 := h z x y y
     have e60 := h z x y z
     have e61 := h z x z x
     have e62 := h z x z y
     have e63 := h z x z z
     have e64 := h z y x x
     have e65 := h z y x y
     have e66 := h z y x z
     have e67 := h z y y x
     have e68 := h z y y y
     have e69 := h z y y z
     have e70 := h z y z x
     have e71 := h z y z y
     have e72 := h z y z z
     have e73 := h z z x x
     have e74 := h z z x y
     have e75 := h z z x z
     have e76 := h z z y x
     have e77 := h z z y y
     have e78 := h z z y z
     have e79 := h z z z x
     have e80 := h z z z y
     have e81 := h z z z z
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q8424930_apply M K])

/-- `x = x ◇ (x ◇ x)` -/
theorem Equation8_StructuralFrom_Equation3483 : Law8.StructuralFrom Law3483 := by
  intro G M hM
  have h : Equation3483 G := Law3483.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3483 h)⟩
  refine structural_q7038766 M K ?_
  rw [@Law8.models_iff]
  intro x
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind [q7038766_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7038766_dg M K, K.dsq]
     try simp only [q7038766_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7038766_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7038766_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x (M.op x x)
     have e3 := h x (M.op x x) x
     have e4 := h x (M.op x x) (M.op x x)
     have e5 := h (M.op x x) x x
     have e6 := h (M.op x x) x (M.op x x)
     have e7 := h (M.op x x) (M.op x x) x
     have e8 := h (M.op x x) (M.op x x) (M.op x x)
     grind [q7038766_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x (M.op x x)
     have e3 := h x (M.op x x) x
     have e4 := h x (M.op x x) (M.op x x)
     have e5 := h (M.op x x) x x
     have e6 := h (M.op x x) x (M.op x x)
     have e7 := h (M.op x x) (M.op x x) x
     have e8 := h (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7038766_apply M K])

/-- `x = x ◇ (y ◇ y)` -/
theorem Equation11_StructuralFrom_Equation3483 : Law11.StructuralFrom Law3483 := by
  intro G M hM
  have h : Equation3483 G := Law3483.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3483 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law11.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ (x ◇ (x ◇ (x ◇ x)))` -/
theorem Equation411_StructuralFrom_Equation3483 : Law411.StructuralFrom Law3483 := by
  intro G M hM
  have h : Equation3483 G := Law3483.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3483 h)⟩
  refine structural_q7038766 M K ?_
  rw [@Law411.models_iff]
  intro x
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind [q7038766_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7038766_dg M K, K.dsq]
     try simp only [q7038766_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7038766_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7038766_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x (M.op x x)
     have e3 := h x (M.op x x) x
     have e4 := h x (M.op x x) (M.op x x)
     have e5 := h (M.op x x) x x
     have e6 := h (M.op x x) x (M.op x x)
     have e7 := h (M.op x x) (M.op x x) x
     have e8 := h (M.op x x) (M.op x x) (M.op x x)
     grind [q7038766_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x (M.op x x)
     have e3 := h x (M.op x x) x
     have e4 := h x (M.op x x) (M.op x x)
     have e5 := h (M.op x x) x x
     have e6 := h (M.op x x) x (M.op x x)
     have e7 := h (M.op x x) (M.op x x) x
     have e8 := h (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7038766_apply M K])

/-- `x = x ◇ (x ◇ (x ◇ (y ◇ y)))` -/
theorem Equation414_StructuralFrom_Equation3483 : Law414.StructuralFrom Law3483 := by
  intro G M hM
  have h : Equation3483 G := Law3483.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3483 h)⟩
  refine structural_q7058366 M K ?_
  rw [@Law414.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind [q7058366_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7058366_dg M K, K.dsq]
     try simp only [q7058366_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7058366_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7058366_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind [q7058366_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7058366_apply M K])

/-- `x = x ◇ (y ◇ (y ◇ (x ◇ x)))` -/
theorem Equation436_StructuralFrom_Equation3483 : Law436.StructuralFrom Law3483 := by
  intro G M hM
  have h : Equation3483 G := Law3483.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3483 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law436.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ (y ◇ (y ◇ (y ◇ y)))` -/
theorem Equation440_StructuralFrom_Equation3483 : Law440.StructuralFrom Law3483 := by
  intro G M hM
  have h : Equation3483 G := Law3483.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3483 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law440.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ ((x ◇ x) ◇ (x ◇ x))` -/
theorem Equation817_StructuralFrom_Equation3483 : Law817.StructuralFrom Law3483 := by
  intro G M hM
  have h : Equation3483 G := Law3483.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3483 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law817.models_iff]
  intro x
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x (M.op x x)
     have e3 := h x (M.op x x) x
     have e4 := h x (M.op x x) (M.op x x)
     have e5 := h (M.op x x) x x
     have e6 := h (M.op x x) x (M.op x x)
     have e7 := h (M.op x x) (M.op x x) x
     have e8 := h (M.op x x) (M.op x x) (M.op x x)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x (M.op x x)
     have e3 := h x (M.op x x) x
     have e4 := h x (M.op x x) (M.op x x)
     have e5 := h (M.op x x) x x
     have e6 := h (M.op x x) x (M.op x x)
     have e7 := h (M.op x x) (M.op x x) x
     have e8 := h (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ ((x ◇ x) ◇ (y ◇ y))` -/
theorem Equation820_StructuralFrom_Equation3483 : Law820.StructuralFrom Law3483 := by
  intro G M hM
  have h : Equation3483 G := Law3483.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3483 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law820.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ ((x ◇ y) ◇ (x ◇ y))` -/
theorem Equation823_StructuralFrom_Equation3483 : Law823.StructuralFrom Law3483 := by
  intro G M hM
  have h : Equation3483 G := Law3483.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3483 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law823.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ ((y ◇ x) ◇ (y ◇ x))` -/
theorem Equation835_StructuralFrom_Equation3483 : Law835.StructuralFrom Law3483 := by
  intro G M hM
  have h : Equation3483 G := Law3483.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3483 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law835.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ ((y ◇ y) ◇ (x ◇ x))` -/
theorem Equation842_StructuralFrom_Equation3483 : Law842.StructuralFrom Law3483 := by
  intro G M hM
  have h : Equation3483 G := Law3483.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3483 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law842.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ ((y ◇ y) ◇ (y ◇ y))` -/
theorem Equation846_StructuralFrom_Equation3483 : Law846.StructuralFrom Law3483 := by
  intro G M hM
  have h : Equation3483 G := Law3483.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3483 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law846.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

/-- `x = x ◇ ((x ◇ (x ◇ x)) ◇ x)` -/
theorem Equation1020_StructuralFrom_Equation3483 : Law1020.StructuralFrom Law3483 := by
  intro G M hM
  have h : Equation3483 G := Law3483.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3483 h)⟩
  refine structural_q7038766 M K ?_
  rw [@Law1020.models_iff]
  intro x
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind [q7038766_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7038766_dg M K, K.dsq]
     try simp only [q7038766_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7038766_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7038766_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x (M.op x x)
     have e3 := h x (M.op x x) x
     have e4 := h x (M.op x x) (M.op x x)
     have e5 := h (M.op x x) x x
     have e6 := h (M.op x x) x (M.op x x)
     have e7 := h (M.op x x) (M.op x x) x
     have e8 := h (M.op x x) (M.op x x) (M.op x x)
     grind [q7038766_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x (M.op x x)
     have e3 := h x (M.op x x) x
     have e4 := h x (M.op x x) (M.op x x)
     have e5 := h (M.op x x) x x
     have e6 := h (M.op x x) x (M.op x x)
     have e7 := h (M.op x x) (M.op x x) x
     have e8 := h (M.op x x) (M.op x x) (M.op x x)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7038766_apply M K])

/-- `x = x ◇ ((x ◇ (y ◇ y)) ◇ x)` -/
theorem Equation1028_StructuralFrom_Equation3483 : Law1028.StructuralFrom Law3483 := by
  intro G M hM
  have h : Equation3483 G := Law3483.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3483 h)⟩
  refine structural_q7038766 M K ?_
  rw [@Law1028.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind [q7038766_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7038766_dg M K, K.dsq]
     try simp only [q7038766_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7038766_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7038766_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind [q7038766_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7038766_apply M K])

/-- `x = x ◇ ((y ◇ (x ◇ x)) ◇ y)` -/
theorem Equation1036_StructuralFrom_Equation3483 : Law1036.StructuralFrom Law3483 := by
  intro G M hM
  have h : Equation3483 G := Law3483.models_iff.mp hM
  obtain ⟨K⟩ : Nonempty (DKer M) := ⟨dkernel M (d3483 h)⟩
  refine structural_q7045822 M K ?_
  rw [@Law1036.models_iff]
  intro x y
  have h2 : ∀ p : G, K.d (M.op p p) := K.dsq
  have h3 : ∀ p q : G, K.d q → M.op p q = M.op q q := K.hin
  first
  | (set_option maxHeartbeats 1500000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 500000 in
     try simp only [q7045822_dg M K, K.dsq]
     try simp only [q7045822_apply M K]
     first
     | done
     | grind
     | (split_ifs <;> simp only [q7045822_apply M K] at * <;> grind)
     | split_ifs <;> grind)
  | (set_option maxHeartbeats 4000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     grind (config := { splits := 24, ematch := 14, instances := 8000, gen := 16 })
       [q7045822_apply M K])
  | (set_option maxHeartbeats 5000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind [q7045822_apply M K])
  | (set_option maxHeartbeats 8000000 in
     have h4 : ∀ q : G, K.d q → ∃ p : G, M.op p p = q := K.dex
     have g1 := f3483_sqeq h K
     have g2 := f3483_sqidem h K
     have g3 := f3483_sqfixl h K
     have g4 := f3483_sqsqeq h K
     have e1 := h x x x
     have e2 := h x x y
     have e3 := h x x (M.op x x)
     have e4 := h x x (M.op y y)
     have e5 := h x y x
     have e6 := h x y y
     have e7 := h x y (M.op x x)
     have e8 := h x y (M.op y y)
     have e9 := h x (M.op x x) x
     have e10 := h x (M.op x x) y
     have e11 := h x (M.op x x) (M.op x x)
     have e12 := h x (M.op x x) (M.op y y)
     have e13 := h x (M.op y y) x
     have e14 := h x (M.op y y) y
     have e15 := h x (M.op y y) (M.op x x)
     have e16 := h x (M.op y y) (M.op y y)
     have e17 := h y x x
     have e18 := h y x y
     have e19 := h y x (M.op x x)
     have e20 := h y x (M.op y y)
     have e21 := h y y x
     have e22 := h y y y
     have e23 := h y y (M.op x x)
     have e24 := h y y (M.op y y)
     have e25 := h y (M.op x x) x
     have e26 := h y (M.op x x) y
     have e27 := h y (M.op x x) (M.op x x)
     have e28 := h y (M.op x x) (M.op y y)
     have e29 := h y (M.op y y) x
     have e30 := h y (M.op y y) y
     have e31 := h y (M.op y y) (M.op x x)
     have e32 := h y (M.op y y) (M.op y y)
     have e33 := h (M.op x x) x x
     have e34 := h (M.op x x) x y
     have e35 := h (M.op x x) x (M.op x x)
     have e36 := h (M.op x x) x (M.op y y)
     have e37 := h (M.op x x) y x
     have e38 := h (M.op x x) y y
     have e39 := h (M.op x x) y (M.op x x)
     have e40 := h (M.op x x) y (M.op y y)
     have e41 := h (M.op x x) (M.op x x) x
     have e42 := h (M.op x x) (M.op x x) y
     have e43 := h (M.op x x) (M.op x x) (M.op x x)
     have e44 := h (M.op x x) (M.op x x) (M.op y y)
     have e45 := h (M.op x x) (M.op y y) x
     have e46 := h (M.op x x) (M.op y y) y
     have e47 := h (M.op x x) (M.op y y) (M.op x x)
     have e48 := h (M.op x x) (M.op y y) (M.op y y)
     have e49 := h (M.op y y) x x
     have e50 := h (M.op y y) x y
     have e51 := h (M.op y y) x (M.op x x)
     have e52 := h (M.op y y) x (M.op y y)
     have e53 := h (M.op y y) y x
     have e54 := h (M.op y y) y y
     have e55 := h (M.op y y) y (M.op x x)
     have e56 := h (M.op y y) y (M.op y y)
     have e57 := h (M.op y y) (M.op x x) x
     have e58 := h (M.op y y) (M.op x x) y
     have e59 := h (M.op y y) (M.op x x) (M.op x x)
     have e60 := h (M.op y y) (M.op x x) (M.op y y)
     have e61 := h (M.op y y) (M.op y y) x
     have e62 := h (M.op y y) (M.op y y) y
     have e63 := h (M.op y y) (M.op y y) (M.op x x)
     have e64 := h (M.op y y) (M.op y y) (M.op y y)
     grind (config := { splits := 20, ematch := 12, instances := 1500, gen := 14 })
       [q7045822_apply M K])

end Law.MagmaLaw
