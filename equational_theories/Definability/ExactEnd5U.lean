...
/-!
# The exact-endomorphism class `U` on `Fin 5`
...
/-! ### The group of units, of order `4`: `⟨(0 1)(3 4), (3 4)⟩`

`End(M)` is `15` maps, so `Aut(M)` -- its unit group -- is these `4` permutations, and the
magmas of the class are among the ones invariant under them. That is what makes the class
enumerable at all: the invariant operations are `11` orbit values rather than `25`
independent cells. -/

/-- The `4` elements of the group: `()`, `(3 4)`, `(0 1)`, `(0 1)(3 4)`. -/
def pE5U0 : Fin 5 → Fin 5 := id
def pE5U1 : Fin 5 → Fin 5 | 0 => 0 | 1 => 1 | 2 => 2 | 3 => 4 | 4 => 3
def pE5U2 : Fin 5 → Fin 5 | 0 => 1 | 1 => 0 | 2 => 2 | 3 => 3 | 4 => 4
def pE5U3 : Fin 5 → Fin 5 | 0 => 1 | 1 => 0 | 2 => 2 | 3 => 4 | 4 => 3

def permE5U : Fin 4 → Fin 5 → Fin 5
  | 0 => pE5U0
  | 1 => pE5U1
  | 2 => pE5U2
  | 3 => pE5U3
...
/-- Indices of the `15` self-maps that must be endomorphisms: `00000`, `11000`, `22000`, `00111`,
`11111`, `22111`, `00222`, `11222`, `22222`, `10243`, `01243`, `22243`, ... (each written as its
list of values at `0, 1, ..., 4`). -/
def eidx5U : Fin 15 → Fin 3125
  | 0 => 0
  | 1 => 6
  | 2 => 12
  | 3 => 775
...
def endE5U (j : Fin 15) : Fin 5 → Fin 5 := allMaps5 (eidx5U j)

/-- Indices of the other `3110`, which must not be endomorphisms. Neither this nor `xidx` is ever
evaluated by the kernel: `endX` occurs only in `native_decide` goals and under `Magma.isExact_iff`,
which is a rewrite. -/
def xarr5U : Array Nat := #[
  ...
  765, 766, 767, 768, 769, 770, 771, 772, 773, 774, 776, 777, 778, 779, 780, 782, 783, 784, 785,
  786, 788, 789, 790, 791, 792, 793, 794, 795, 796, 797, 798, 799, 800, 801, 802, 803, 804, 805,
  ...
  1540, 1541, 1542, 1543, 1544, 1545, 1546, 1547, 1548, 1549, 1551, 1552, 1553, 1554, 1555, 1557,
  1558, 1559, 1560, 1561, 1563, 1564, 1565, 1566, 1567, 1568, 1569, 1570, 1571, 1572, 1573, 1574,
  ...
  2423, 2424, 2425, 2427, 2428, 2429, 2431, 2432, 2433, 2434, 2435, 2436, 2438, 2439, 2440, 2441,
  ...
  2922, 2923, 2924, 2925, 2927, 2928, 2929, 2931, 2932, 2933, 2934, 2935, 2936, 2938, 2939, 2940,
...
def xidx5U (j : Fin 3110) : Fin 3125 :=
  ⟨min (xarr5U.getD j.1 0) 3124, by omega⟩

def endX5U (j : Fin 3110) : Fin 5 → Fin 5 := allMaps5 (xidx5U j)

/-- Where each element of the group sits among the demanded endomorphisms, so that the orbit-
transport classification can be run on the hypothesis `end5U` is given. -/
def uidx5U : Fin 4 → Fin 15
  | 0 => 13
  | 1 => 10
  | 2 => 12
  | 3 => 9

theorem permE5U_eq_endE5U (i : Fin 4) : permE5U i = endE5U (uidx5U i) := by
  revert i; decide

/-- One representative per orbit of the group on the `25` cells; there are `11`, so an invariant
operation is `11` values. -/
def rep5U : Fin 11 → Fin 5 × Fin 5
  | 0 => (0, 0)
  | 1 => (0, 1)
  | 2 => (0, 2)
  | 3 => (0, 3)
...
def tr5U : Fin 5 → Fin 5 → Fin 11 × Fin 4
  | 0, 0 => (0, 0)
  | 0, 1 => (1, 0)
  | 0, 2 => (2, 0)
  | 0, 3 => (3, 0)
...
def stabIdx5U0 : Fin 2 → Fin 4
  | 0 => 0
  | 1 => 1
def val5U0 : Fin 3 → Fin 5
  | 0 => 0
  | 1 => 1
  | 2 => 2
...
value: that value must be one of `0`, `1`, `2`, so the orbit is indexed by `Fin 3` instead. -/
def stabIdx5U1 : Fin 2 → Fin 4
  | 0 => 0
  | 1 => 1
def val5U1 : Fin 3 → Fin 5
  | 0 => 0
  | 1 => 1
  | 2 => 2
...
value: that value must be one of `0`, `1`, `2`, so the orbit is indexed by `Fin 3` instead. -/
def stabIdx5U2 : Fin 2 → Fin 4
  | 0 => 0
  | 1 => 1
def val5U2 : Fin 3 → Fin 5
  | 0 => 0
  | 1 => 1
  | 2 => 2
...
def stabIdx5U5 : Fin 4 → Fin 4
  | 0 => 0
  | 1 => 1
  | 2 => 2
  | 3 => 3
def val5U5 : Fin 1 → Fin 5
  | 0 => 2

/-- The `2` group elements fixing both coordinates of orbit `6`'s representative, and hence its
value: that value must be one of `2`, `3`, `4`, so the orbit is indexed by `Fin 3` instead. -/
def stabIdx5U6 : Fin 2 → Fin 4
  | 0 => 0
  | 1 => 2
def val5U6 : Fin 3 → Fin 5
  | 0 => 2
  | 1 => 3
  | 2 => 4
...
value: that value must be one of `2`, `3`, `4`, so the orbit is indexed by `Fin 3` instead. -/
def stabIdx5U9 : Fin 2 → Fin 4
  | 0 => 0
  | 1 => 2
def val5U9 : Fin 3 → Fin 5
  | 0 => 2
  | 1 => 3
  | 2 => 4
...
value: that value must be one of `2`, `3`, `4`, so the orbit is indexed by `Fin 3` instead. -/
def stabIdx5U10 : Fin 2 → Fin 4
  | 0 => 0
  | 1 => 2
def val5U10 : Fin 3 → Fin 5
  | 0 => 2
  | 1 => 3
  | 2 => 4

/-- The `11` orbit values named by the family's arguments. -/
def dec5U (a b c : Fin 3) (d : Fin 5) (e : Fin 3) (f : Fin 1) (g : Fin 3) (h : Fin 5)
    (i j k : Fin 3) : Fin 11 → Fin 5
  | 0 => val5U0 a
  | 1 => val5U1 b
  | 2 => val5U2 c
  | 3 => d
...
`Magma.transport` specialised (`tab5U_eq_transport`). -/
def tab5U (a b c : Fin 3) (d : Fin 5) (e : Fin 3) (f : Fin 1) (g : Fin 3) (h : Fin 5)
    (i j k : Fin 3) : Fin 5 → Fin 5 → Fin 5
  | 0, 0 => val5U0 a
  | 0, 1 => val5U1 b
  | 0, 2 => val5U2 c
  | 0, 3 => d
...
theorem tab5U_eq_transport (a b c : Fin 3) (d : Fin 5) (e : Fin 3) (f : Fin 1) (g : Fin 3)
    (h : Fin 5) (i j k : Fin 3) :
    tab5U a b c d e f g h i j k = Magma.transport permE5U tr5U (dec5U a b c d e f g h i j k) := by
  funext x y; fin_cases x <;> fin_cases y <;> rfl


/-- The `22` magmas on `Fin 5` whose endomorphism monoid is exactly this one, listed under one
index. Of the `164025` invariant tuples these are the ones the forbidden maps rule out; naming only
them keeps the exactness test off the hot path of a family scan. -/
def mem5U : Fin 22 → Fin 5 → Fin 5 → Fin 5
  | 0 => tab5U 0 0 0 1 2 0 0 2 0 0 1
  | 1 => tab5U 0 0 0 1 2 0 0 2 0 0 2
  | 2 => tab5U 0 0 0 1 2 0 0 2 0 2 0
  | 3 => tab5U 0 0 0 1 2 0 0 2 0 2 1
  | 4 => tab5U 0 0 0 1 2 0 0 2 0 2 2
  | 5 => tab5U 0 0 0 1 2 0 0 3 1 0 2
  | 6 => tab5U 0 0 0 1 2 0 0 3 1 2 0
  | 7 => tab5U 0 0 0 1 2 0 0 4 2 0 0
  | 8 => tab5U 0 0 0 1 2 0 0 4 2 0 1
  | 9 => tab5U 0 0 0 1 2 0 0 4 2 0 2
  | 10 => tab5U 0 0 0 1 2 0 0 4 2 2 0
  | 11 => tab5U 0 1 2 2 0 0 0 1 0 0 1
  | 12 => tab5U 0 1 2 2 0 0 0 1 0 0 2
  | 13 => tab5U 0 1 2 2 0 0 0 1 0 2 0
  | 14 => tab5U 0 1 2 2 0 0 0 1 0 2 1
  | 15 => tab5U 0 1 2 2 0 0 0 1 0 2 2
  | 16 => tab5U 0 1 2 3 0 0 1 1 0 0 1
  | 17 => tab5U 0 1 2 3 0 0 1 1 0 2 0
  | 18 => tab5U 0 1 2 4 0 0 2 1 0 0 0
  | 19 => tab5U 0 1 2 4 0 0 2 1 0 0 1
  | 20 => tab5U 0 1 2 4 0 0 2 1 0 0 2
  | _ => tab5U 0 1 2 4 0 0 2 1 0 2 0

/-- The arguments as one mixed-radix index, which is how `rankIdx5U` is addressed. -/
def encT5U (a b c : Fin 3) (d : Fin 5) (e : Fin 3) (f : Fin 1) (g : Fin 3) (h : Fin 5)
    (i j k : Fin 3) : Nat :=
  a.1 * 54675 +
    b.1 * 18225 +
    c.1 * 6075 +
...
/-- Which of the `22` members a given invariant tuple is, so that `mem_of_isExact5U` can name one
rather than search the list for it. The `164003` tuples that are not members fall through to `0`;
the hypothesis of `mem_rank5U` excludes them. Read only by `native_decide`. -/
def rankIdx5U : Nat → Nat
  | 2080 => 0
  | 2081 => 1
  | 2085 => 2
  | 2086 => 3
...
def rank5U (a b c : Fin 3) (d : Fin 5) (e : Fin 3) (f : Fin 1) (g : Fin 3) (h : Fin 5)
    (i j k : Fin 3) : Fin 22 :=
  ⟨min (rankIdx5U (encT5U a b c d e f g h i j k)) 21, by omega⟩


/-- The same as a family of magmas, which is what `FamilyRefutes` scans. -/
@[implicit_reducible]
def end5U (i : Fin 22) : Magma (Fin 5) := Magma.mk (mem5U i)

theorem end5U_isEndo (i : Fin 22) (j : Fin 15) : (end5U i).IsEndo (endE5U j) := by
  revert i j; native_decide

theorem end5U_not_isEndo (i : Fin 22) (j : Fin 3110) : ¬ (end5U i).IsEndo (endX5U j) := by
  revert i j; native_decide

/-- `rank5U` really does name the member a tuple is. Deciding this is one array read and one table
comparison per tuple, where searching `mem5U` for the index would be a comparison per tuple and
member. -/
theorem mem_rank5U (a b c : Fin 3) (d : Fin 5) (e : Fin 3) (f : Fin 1) (g : Fin 3) (h : Fin 5)
    (i j k : Fin 3)
    (hx : Magma.isExact (tab5U a b c d e f g h i j k) endE5U endX5U = true) :
    mem5U (rank5U a b c d e f g h i j k) = tab5U a b c d e f g h i j k := by
...
members. -/
theorem mem_of_isExact5U (a b c : Fin 3) (d : Fin 5) (e : Fin 3) (f : Fin 1) (g : Fin 3) (h : Fin 5)
    (i j k : Fin 3)
    (hx : Magma.isExact (tab5U a b c d e f g h i j k) endE5U endX5U = true) :
    ∃ idx, mem5U idx = tab5U a b c d e f g h i j k :=
  ⟨_, mem_rank5U a b c d e f g h i j k hx⟩

/-- A magma on `Fin 5` admitting every demanded map and none of the forbidden ones *is* a member of
`Magma.end5U`. The demanded maps include the group, so its values at the orbit representatives name
an invariant tuple, and `mem_of_isExact5U` turns that into a member index. -/
theorem exists_end5U {M : Magma (Fin 5)} (hE : ∀ j, M.IsEndo (endE5U j))
    (hX : ∀ j, ¬ M.IsEndo (endX5U j)) : ∃ i, end5U i = M := by
  have hP : ∀ i, M.IsEndo (permE5U i) := by
    intro i; rw [permE5U_eq_endE5U]; exact hE (uidx5U i)
  have hop : M.op = Magma.transport permE5U tr5U (fun i ↦ M.op (rep5U i).1 (rep5U i).2) :=
    Magma.op_eq_transport (rep := rep5U) hP (by decide)
  obtain ⟨a, ha⟩ := Magma.exists_val_of_isEndo
    (E := permE5U) (idx := stabIdx5U0) (a := (rep5U 0).1) (b := (rep5U 0).2)
    (val := val5U0) hP (by decide) (by decide)
  obtain ⟨b, hb⟩ := Magma.exists_val_of_isEndo
    (E := permE5U) (idx := stabIdx5U1) (a := (rep5U 1).1) (b := (rep5U 1).2)
    (val := val5U1) hP (by decide) (by decide)
  obtain ⟨c, hc⟩ := Magma.exists_val_of_isEndo
    (E := permE5U) (idx := stabIdx5U2) (a := (rep5U 2).1) (b := (rep5U 2).2)
    (val := val5U2) hP (by decide) (by decide)
  obtain ⟨d, hd⟩ := Magma.exists_val_of_isEndo
    (E := permE5U) (idx := stabIdx5U3) (a := (rep5U 3).1) (b := (rep5U 3).2)
    (val := id) hP (by decide) (by decide)
  obtain ⟨e, he⟩ := Magma.exists_val_of_isEndo
    (E := permE5U) (idx := stabIdx5U4) (a := (rep5U 4).1) (b := (rep5U 4).2)
    (val := val5U4) hP (by decide) (by decide)
  obtain ⟨f, hf⟩ := Magma.exists_val_of_isEndo
    (E := permE5U) (idx := stabIdx5U5) (a := (rep5U 5).1) (b := (rep5U 5).2)
    (val := val5U5) hP (by decide) (by decide)
  obtain ⟨g, hg⟩ := Magma.exists_val_of_isEndo
    (E := permE5U) (idx := stabIdx5U6) (a := (rep5U 6).1) (b := (rep5U 6).2)
    (val := val5U6) hP (by decide) (by decide)
  obtain ⟨h, hh⟩ := Magma.exists_val_of_isEndo
    (E := permE5U) (idx := stabIdx5U7) (a := (rep5U 7).1) (b := (rep5U 7).2)
    (val := id) hP (by decide) (by decide)
  obtain ⟨i, hi⟩ := Magma.exists_val_of_isEndo
    (E := permE5U) (idx := stabIdx5U8) (a := (rep5U 8).1) (b := (rep5U 8).2)
    (val := val5U8) hP (by decide) (by decide)
  obtain ⟨j, hj⟩ := Magma.exists_val_of_isEndo
    (E := permE5U) (idx := stabIdx5U9) (a := (rep5U 9).1) (b := (rep5U 9).2)
    (val := val5U9) hP (by decide) (by decide)
  obtain ⟨k, hk⟩ := Magma.exists_val_of_isEndo
    (E := permE5U) (idx := stabIdx5U10) (a := (rep5U 10).1) (b := (rep5U 10).2)
    (val := val5U10) hP (by decide) (by decide)
  have hdec : dec5U a b c d e f g h i j k = fun i ↦ M.op (rep5U i).1 (rep5U i).2 := by
    ...
    · exact hj
    · exact hk
  have key : tab5U a b c d e f g h i j k = M.op := by
    rw [tab5U_eq_transport, hdec, ← hop]
  have hok : Magma.isExact (tab5U a b c d e f g h i j k) endE5U endX5U = true := by
    rw [key, Magma.isExact_iff]; exact ⟨hE, hX⟩
  obtain ⟨idx, hidx⟩ := mem_of_isExact5U a b c d e f g h i j k hok
...
monoid is exactly the order-`15` monoid of `Magma.endE5U`, and no such magma satisfies `L`, then `L`
is not term-structural from `L'`, not even over finite carriers. -/
theorem not_termStructuralFromFin_of_end5U {L L' : Law.NatMagmaLaw} {i : Fin 22}
    (hsrc : @satisfies _ (Fin 5) (Magma.end5U i) L')
    (htgt : ∀ j, ¬ @satisfies _ (Fin 5) (Magma.end5U j) L) :
    ¬ L.TermStructuralFromFin L' := by
  intro h
  obtain ⟨M', hM', hfwd, hback⟩ := h (Magma.end5U i) hsrc
  obtain ⟨j, hM⟩ := Magma.exists_end5U
    (fun k ↦ (Magma.end5U_isEndo i k).of_termDefinable hfwd)
    (fun k hn ↦ Magma.end5U_not_isEndo i k (hn.of_termDefinable hback))
