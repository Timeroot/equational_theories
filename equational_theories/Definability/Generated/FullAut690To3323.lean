import DefOrbit.Generated.NativeFullAut690To3323
...
namespace Magma.FullAut690To3323
...
private theorem native_orbit_correct : ∀ x y : Fin 12,
    DefOrbit.NativeFullAut690To3323.orbitData.getD (x.val * 12 + y.val) 0 = (tr x y).1.val := by native_decide
private theorem native_places_correct : ∀ j : Fin 9,
    DefOrbit.NativeFullAut690To3323.placesData.getD j.val 0 = places j := by native_decide
private theorem native_sizes_correct : ∀ j : Fin 9,
    DefOrbit.NativeFullAut690To3323.sizesData.getD j.val 0 = sizes j := by native_decide
private theorem native_cell_correct : ∀ (x y : Fin 12) (v : Fin (sizes (tr x y).1)),
    (DefOrbit.NativeFullAut690To3323.cellData.getD (x.val * 12 + y.val) #[]).getD v.val 0 =
      image (tr x y).2 (values (tr x y).1 v) := by native_decide
private theorem native_op_correct (i : Fin 8748 × Fin 9216) (x y : Fin 12) :
    DefOrbit.NativeFullAut690To3323.op (i.2.val + 9216 * i.1.val) x y = (fam (paramEquiv.symm i)).op x y := by
  rw [← decodeNative_eq]
  unfold DefOrbit.NativeFullAut690To3323.op
  dsimp only
...
private theorem native_probe_correct : ∀ i : Fin 2, ∀ x : Fin 12,
    DefOrbit.NativeFullAut690To3323.probe i x = probePerm i x := by native_decide
private theorem native_pattern_correct (i : Fin 8748 × Fin 9216) :
    DefOrbit.NativeFullAut690To3323.pattern (i.2.val + 9216 * i.1.val) ↔ Pattern (fam (paramEquiv.symm i)) probePerm pattern := by
  unfold DefOrbit.NativeFullAut690To3323.pattern Pattern pattern
  simp only [native_op_correct, native_probe_correct, Magma.IsEndo]
  simp

private abbrev test3323 (p : Params) : Prop := ∀ y z : Fin 12, ((fam p).op 0 y) = ((fam p).op 0 ((fam p).op y ((fam p).op z z)))
private theorem native_test3323_correct (i : Fin 8748 × Fin 9216) :
    DefOrbit.NativeFullAut690To3323.test3323 (i.2.val + 9216 * i.1.val) ↔ test3323 (paramEquiv.symm i) := by
  unfold DefOrbit.NativeFullAut690To3323.test3323 test3323
  simp only [native_op_correct]
private theorem native_checked3323 : DefOrbit.allBelow DefOrbit.NativeFullAut690To3323.check3323 80621568 = true := by
  native_decide
private theorem restricted3323 : ∀ p : Params, test3323 p → ¬ Pattern (fam p) probePerm pattern := by
  have h : ∀ i : Fin 8748 × Fin 9216, test3323 (paramEquiv.symm i) →
      ¬ Pattern (fam (paramEquiv.symm i)) probePerm pattern := by
    intro i
    have hi : i.2.val + 9216 * i.1.val < 80621568 := by omega
    have hc := (DefOrbit.allBelow_spec _ _).mp native_checked3323 _ hi
    have ht := of_decide_eq_true hc
    change DefOrbit.NativeFullAut690To3323.test3323 (i.2.val + 9216 * i.1.val) → ¬ DefOrbit.NativeFullAut690To3323.pattern (i.2.val + 9216 * i.1.val) at ht
    simpa only [native_test3323_correct, native_pattern_correct] using ht
  intro p
  simpa only [Equiv.symm_apply_apply] using h (paramEquiv p)
theorem target3323 : ∀ p : Params, @Equation3323 (Fin 12) (fam p) → ¬ Pattern (fam p) probePerm pattern := by
  intro p ht
  exact restricted3323 p (ht 0)

end Magma.FullAut690To3323

open Magma.FullAut690To3323

theorem Equation3323_not_structuralFromFin_Equation690_fullAut690To3323 : ¬ Law3323.StructuralFromFin Law690 := by
  ...
  · intro p hp
    exact target3323 p ((@Law3323.models_iff (Fin 12) (fam p)).mp hp)

/-- info: 'Equation3323_not_structuralFromFin_Equation690_fullAut690To3323' depends on axioms: [propext,
 Classical.choice,
 Quot.sound,
 Equation3323_not_structuralFromFin_Equation690_fullAut690To3323._native.native_decide.ax_1_1,
 ...
 native_checked3323._native.native_decide.ax_1_1,
...
#print axioms Equation3323_not_structuralFromFin_Equation690_fullAut690To3323
