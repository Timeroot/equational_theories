import DefOrbit.Generated.NativeFullAut690To3523
...
namespace Magma.FullAut690To3523
...
def probeInvData : Array (Array (Fin 12)) := #[#[0, 1, 2, 5, 3, 4, 6, 7, 8, 11, 9, 10], #[0, 1, 2, 3, 10, 5, 6, 7, 8, 9, 4, 11]]
def probeInv (i : Fin 2) (x : Fin 12) : Fin 12 := (probeInvData.getD i.val #[]).getD x.val 0

def sourceRowData : Array (Array (Fin 12)) := #[#[2, 0, 1, 5, 9, 10, 8, 6, 7, 11, 3, 4], #[2, 0, 1, 11, 3, 10, 8, 6, 7, 5, 9, 4], #[2, 0, 1, 11, 9, 4, 8, 6, 7, 5, 3, 10], #[2, 6, 7, 5, 3, 4, 8, 0, 1, 11, 9, 10], #[8, 0, 7, 5, 3, 4, 2, 6, 1, 11, 9, 10], #[8, 6, 1, 5, 3, 4, 2, 0, 7, 11, 9, 10], #[2, 0, 1, 5, 9, 10, 8, 6, 7, 11, 3, 4], #[2, 0, 1, 11, 3, 10, 8, 6, 7, 5, 9, 4], #[2, 0, 1, 11, 9, 4, 8, 6, 7, 5, 3, 10], #[2, 6, 7, 5, 3, 4, 8, 0, 1, 11, 9, 10], #[8, 0, 7, 5, 3, 4, 2, 6, 1, 11, 9, 10], #[8, 6, 1, 5, 3, 4, 2, 0, 7, 11, 9, 10]]
...
private theorem native_orbit_correct : ∀ x y : Fin 12,
    DefOrbit.NativeFullAut690To3523.orbitData.getD (x.val * 12 + y.val) 0 = (tr x y).1.val := by native_decide
private theorem native_places_correct : ∀ j : Fin 9,
    DefOrbit.NativeFullAut690To3523.placesData.getD j.val 0 = places j := by native_decide
private theorem native_sizes_correct : ∀ j : Fin 9,
    DefOrbit.NativeFullAut690To3523.sizesData.getD j.val 0 = sizes j := by native_decide
private theorem native_cell_correct : ∀ (x y : Fin 12) (v : Fin (sizes (tr x y).1)),
    (DefOrbit.NativeFullAut690To3523.cellData.getD (x.val * 12 + y.val) #[]).getD v.val 0 =
      image (tr x y).2 (values (tr x y).1 v) := by native_decide
private theorem native_op_correct (i : Fin 8748 × Fin 9216) (x y : Fin 12) :
    DefOrbit.NativeFullAut690To3523.op (i.2.val + 9216 * i.1.val) x y = (fam (paramEquiv.symm i)).op x y := by
  rw [← decodeNative_eq]
  unfold DefOrbit.NativeFullAut690To3523.op
  dsimp only
...
private theorem native_probe_correct : ∀ i : Fin 2, ∀ x : Fin 12,
    DefOrbit.NativeFullAut690To3523.probe i x = probePerm i x := by native_decide
private theorem native_pattern_correct (i : Fin 8748 × Fin 9216) :
    DefOrbit.NativeFullAut690To3523.pattern (i.2.val + 9216 * i.1.val) ↔ Pattern (fam (paramEquiv.symm i)) probePerm pattern := by
  unfold DefOrbit.NativeFullAut690To3523.pattern Pattern pattern
  simp only [native_op_correct, native_probe_correct, Magma.IsEndo]
  simp

private abbrev test3523 (p : Params) : Prop := ∀ y z : Fin 12, ((fam p).op 0 y) = ((fam p).op 0 ((fam p).op ((fam p).op y y) z))
private theorem native_test3523_correct (i : Fin 8748 × Fin 9216) :
    DefOrbit.NativeFullAut690To3523.test3523 (i.2.val + 9216 * i.1.val) ↔ test3523 (paramEquiv.symm i) := by
  unfold DefOrbit.NativeFullAut690To3523.test3523 test3523
  simp only [native_op_correct]
private theorem native_checked3523 : DefOrbit.allBelow DefOrbit.NativeFullAut690To3523.check3523 80621568 = true := by
  native_decide
private theorem restricted3523 : ∀ p : Params, test3523 p → ¬ Pattern (fam p) probePerm pattern := by
  have h : ∀ i : Fin 8748 × Fin 9216, test3523 (paramEquiv.symm i) →
      ¬ Pattern (fam (paramEquiv.symm i)) probePerm pattern := by
    intro i
    have hi : i.2.val + 9216 * i.1.val < 80621568 := by omega
    have hc := (DefOrbit.allBelow_spec _ _).mp native_checked3523 _ hi
    have ht := of_decide_eq_true hc
    change DefOrbit.NativeFullAut690To3523.test3523 (i.2.val + 9216 * i.1.val) → ¬ DefOrbit.NativeFullAut690To3523.pattern (i.2.val + 9216 * i.1.val) at ht
    simpa only [native_test3523_correct, native_pattern_correct] using ht
  intro p
  simpa only [Equiv.symm_apply_apply] using h (paramEquiv p)
theorem target3523 : ∀ p : Params, @Equation3523 (Fin 12) (fam p) → ¬ Pattern (fam p) probePerm pattern := by
  intro p ht
  exact restricted3523 p (ht 0)

end Magma.FullAut690To3523

open Magma.FullAut690To3523

theorem Equation3523_not_structuralFromFin_Equation690_fullAut690To3523 : ¬ Law3523.StructuralFromFin Law690 := by
  apply Magma.FiniteSymmetry.not_structuralFromFin source genPerm probePerm pattern fam coverage
  · exact (@Law690.models_iff (Fin 12) source).mpr (by native_decide)
  · exact source_generators
  · exact source_pattern
  · intro p hp
    exact target3523 p ((@Law3523.models_iff (Fin 12) (fam p)).mp hp)

/-- info: 'Equation3523_not_structuralFromFin_Equation690_fullAut690To3523' depends on axioms: [propext,
 Classical.choice,
 Quot.sound,
 Equation3523_not_structuralFromFin_Equation690_fullAut690To3523._native.native_decide.ax_1_1,
 ...
 native_checked3523._native.native_decide.ax_1_1,
...
#print axioms Equation3523_not_structuralFromFin_Equation690_fullAut690To3523
