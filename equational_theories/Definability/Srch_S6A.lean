import equational_theories.Definability.Srch_S6A_data

/-!
# The searched exact-automorphism class on `Fin 6`: the rectangle
...
is `decide` on `Fin 6`; the searching is in `Definability/Certs/Srch6AT*.lean`.
-/
...
theorem not_structuralFromFin_of_srch6A {L L' : Law.NatMagmaLaw} (v : Magma.tupS6A)
    (hsrc : @satisfies _ (Fin 6) (Magma.srch6A v) L')
    (htgt : ∀ u : Magma.tupS6A, ¬ @satisfies _ (Fin 6) (Magma.srch6A u) L) :
    ¬ L.StructuralFromFin L' :=
  Law.MagmaLaw.not_structuralFromFin_of_tupOK S6A.E S6A.Einv S6A.X S6A.Xinv S6A.rep S6A.tr
    S6A.z S6A.st S6A.hl S6A.hr S6A.hxl S6A.hxr S6A.htr S6A.hz S6A.hst v.1 v.2 hsrc htgt
