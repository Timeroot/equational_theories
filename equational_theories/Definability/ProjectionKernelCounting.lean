...
/-! Translation families with a projection and prescribed maps on the kernel. -/
open Law Law.MagmaLaw

namespace Magma.ProjectionKernelFamily
variable {A B K I : Type} [AddCommGroup A] [AddCommGroup B]
...
@[implicit_reducible] def fam (p : Params A K) : Magma (A × B) :=
  ⟨fun x y => x + function values p (y - x)⟩
...
theorem coverage
  ...
  change x + function values _ (y - x) = N.op x y
  rw [hf]
...
theorem family_injective (hv : Function.Injective values) :
  ...
  have hf (z : A × B) : function values p z = function values q z := by
    ...
    change 0 + function values p (z - 0) = 0 + function values q (z - 0) at h
...
theorem family_generators
  ...
  cases i with
  | inl a =>
    change a + (x + function values p (y - x)) =
      (a + x) + function values p ((a + y) - (a + x))
    simp only [add_sub_add_left_eq_sub, add_assoc]
  | inr i =>
    cases i with
    | none =>
      change ((x + function values p (y - x)).1, (0 : B)) =
        (x.1, 0) + function values p ((y.1, 0) - (x.1, 0))
      apply Prod.ext <;> simp [function, hz]
    | some i =>
      change lift (maps i) (x + function values p (y - x)) =
        lift (maps i) x + function values p (lift (maps i) y - lift (maps i) x)
      apply Prod.ext <;> simp [function, lift, hc]

end Magma.ProjectionKernelFamily

/-- info: 'Magma.ProjectionKernelFamily.kernel_coverage_of_chart' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms Magma.ProjectionKernelFamily.kernel_coverage_of_chart
/-- info: 'Magma.ProjectionKernelFamily.coverage' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms Magma.ProjectionKernelFamily.coverage
/-- info: 'Magma.ProjectionKernelFamily.family_injective' depends on axioms: [propext, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms Magma.ProjectionKernelFamily.family_injective
/-- info: 'Magma.ProjectionKernelFamily.family_generators' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms Magma.ProjectionKernelFamily.family_generators
