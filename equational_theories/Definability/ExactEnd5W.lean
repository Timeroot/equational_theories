...
/-!
# The exact-endomorphism class `W` on `Fin 5`
...
def pE5W0 : Fin 5 → Fin 5 := id
def pE5W1 : Fin 5 → Fin 5 | 0 => 0 | 1 => 1 | 2 => 2 | 3 => 4 | 4 => 3
def pE5W2 : Fin 5 → Fin 5 | 0 => 0 | 1 => 3 | 2 => 2 | 3 => 1 | 4 => 4
def pE5W3 : Fin 5 → Fin 5 | 0 => 0 | 1 => 3 | 2 => 2 | 3 => 4 | 4 => 1
def pE5W4 : Fin 5 → Fin 5 | 0 => 0 | 1 => 4 | 2 => 2 | 3 => 1 | 4 => 3
def pE5W5 : Fin 5 → Fin 5 | 0 => 0 | 1 => 4 | 2 => 2 | 3 => 3 | 4 => 1

def permE5W : Fin 6 → Fin 5 → Fin 5
  | 0 => pE5W0
  | 1 => pE5W1
  | 2 => pE5W2
  | 3 => pE5W3
...
/-- Indices of the `10` self-maps that must be endomorphisms: `00000`, `02020`, `04231`, `03241`,
`02002`, `00022`, `04213`, `01243`, `03214`, `01234` (each written as its list of values at `0, 1,
..., 4`). -/
def eidx5W : Fin 10 → Fin 3125
  | 0 => 0
  | 1 => 260
  | 2 => 1070
  | 3 => 1190
...
def endE5W (j : Fin 10) : Fin 5 → Fin 5 := allMaps5 (eidx5W j)
...
def xarr5W : Array Nat := #[
  ...
  251, 252, 253, 254, 255, 256, 257, 258, 259, 261, 262, 263, 264, 265, 266, 267, 268, 269, 270,
  ...
  1252, 1253, 1254, 1255, 1256, 1257, 1258, 1259, 1261, 1262, 1263, 1264, 1265, 1266, 1267, 1268,
  ...
  1493, 1494, 1495, 1496, 1497, 1498, 1499, 1501, 1502, 1503, 1504, 1505, 1506, 1507, 1508, 1509,
  ...
  2680, 2681, 2682, 2683, 2684, 2685, 2686, 2687, 2688, 2689, 2691, 2692, 2693, 2694, 2695, 2696,
...
def xidx5W (j : Fin 3115) : Fin 3125 :=
  ⟨min (xarr5W.getD j.1 0) 3124, by omega⟩

def endX5W (j : Fin 3115) : Fin 5 → Fin 5 := allMaps5 (xidx5W j)

/-- Where each element of the group sits among the demanded endomorphisms, so that the orbit-
transport classification can be run on the hypothesis `end5W` is given. -/
def uidx5W : Fin 6 → Fin 10
  | 0 => 9
  | 1 => 7
  | 2 => 8
  | 3 => 3
...
def tr5W : Fin 5 → Fin 5 → Fin 10 × Fin 6
  | 0, 0 => (0, 0)
  | 0, 1 => (1, 0)
  | 0, 2 => (2, 0)
  | 0, 3 => (1, 2)
...
def stabIdx5W1 : Fin 2 → Fin 6
  | 0 => 0
  | 1 => 1
def val5W1 : Fin 3 → Fin 5
  | 0 => 0
  | 1 => 1
  | 2 => 2
...
value: that value must be one of `0`, `1`, `2`, so the orbit is indexed by `Fin 3` instead. -/
def stabIdx5W4 : Fin 2 → Fin 6
  | 0 => 0
  | 1 => 1
def val5W4 : Fin 3 → Fin 5
  | 0 => 0
  | 1 => 1
  | 2 => 2
...
value: that value must be one of `0`, `1`, `2`, so the orbit is indexed by `Fin 3` instead. -/
def stabIdx5W5 : Fin 2 → Fin 6
  | 0 => 0
  | 1 => 1
def val5W5 : Fin 3 → Fin 5
  | 0 => 0
  | 1 => 1
  | 2 => 2
...
def stabIdx5W9 : Fin 6 → Fin 6
  ...
  | 4 => 4
  | 5 => 5
def val5W9 : Fin 2 → Fin 5
  | 0 => 0
  | 1 => 2

/-- The `10` orbit values named by the family's arguments. -/
def dec5W (a : Fin 2) (b : Fin 3) (c : Fin 2) (d e f : Fin 3) (g : Fin 5) (h : Fin 2) (i : Fin 3)
...
`Magma.transport` specialised (`tab5W_eq_transport`). -/
def tab5W (a : Fin 2) (b : Fin 3) (c : Fin 2) (d e f : Fin 3) (g : Fin 5) (h : Fin 2) (i : Fin 3)
...
theorem tab5W_eq_transport (a : Fin 2) (b : Fin 3) (c : Fin 2) (d e f : Fin 3) (g : Fin 5)
    (h : Fin 2) (i : Fin 3) (j : Fin 2) :
    tab5W a b c d e f g h i j = Magma.transport permE5W tr5W (dec5W a b c d e f g h i j) := by
  funext x y; fin_cases x <;> fin_cases y <;> rfl


/-- The `1` magmas on `Fin 5` whose endomorphism monoid is exactly this one, listed under one index.
Of the `19440` invariant tuples these are the ones the forbidden maps rule out; naming only them
keeps the exactness test off the hot path of a family scan. -/
def mem5W : Fin 1 → Fin 5 → Fin 5 → Fin 5
  | 0 => tab5W 0 1 1 1 2 1 4 1 1 0

/-- Which of the `1` members a given invariant tuple is, so that `mem_of_isExact5W` can name one
rather than search the list for it. Read only by `native_decide`. -/
def rankArr5W : Array Nat := #[
...
/-- The arguments as one mixed-radix index, which is how `rankArr5W` is addressed. -/
def encT5W (a : Fin 2) (b : Fin 3) (c : Fin 2) (d e f : Fin 3) (g : Fin 5) (h : Fin 2) (i : Fin 3)
    (j : Fin 2) : Nat :=
  a.1 * 9720 +
    b.1 * 3240 +
    c.1 * 1620 +
...
def rank5W (a : Fin 2) (b : Fin 3) (c : Fin 2) (d e f : Fin 3) (g : Fin 5) (h : Fin 2) (i : Fin 3)
    (j : Fin 2) : Fin 1 :=
  ⟨min (rankArr5W.getD (encT5W a b c d e f g h i j) 0) 0, by omega⟩


/-- The same as a family of magmas, which is what `FamilyRefutes` scans. -/
@[implicit_reducible]
def end5W (i : Fin 1) : Magma (Fin 5) := Magma.mk (mem5W i)

theorem end5W_isEndo (i : Fin 1) (j : Fin 10) : (end5W i).IsEndo (endE5W j) := by
  revert i j; native_decide

theorem end5W_not_isEndo (i : Fin 1) (j : Fin 3115) : ¬ (end5W i).IsEndo (endX5W j) := by
  revert i j; native_decide

/-- `rank5W` really does name the member a tuple is. Deciding this is one array read and one table
comparison per tuple, where searching `mem5W` for the index would be a comparison per tuple and
member. -/
theorem mem_rank5W (a : Fin 2) (b : Fin 3) (c : Fin 2) (d e f : Fin 3) (g : Fin 5) (h : Fin 2)
    (i : Fin 3) (j : Fin 2)
    (hx : Magma.isExact (tab5W a b c d e f g h i j) endE5W endX5W = true) :
    mem5W (rank5W a b c d e f g h i j) = tab5W a b c d e f g h i j := by
...
members. -/
theorem mem_of_isExact5W (a : Fin 2) (b : Fin 3) (c : Fin 2) (d e f : Fin 3) (g : Fin 5) (h : Fin 2)
    (i : Fin 3) (j : Fin 2)
    (hx : Magma.isExact (tab5W a b c d e f g h i j) endE5W endX5W = true) :
    ∃ idx, mem5W idx = tab5W a b c d e f g h i j :=
  ⟨_, mem_rank5W a b c d e f g h i j hx⟩

/-- A magma on `Fin 5` admitting every demanded map and none of the forbidden ones *is* a member of
`Magma.end5W`. The demanded maps include the group, so its values at the orbit representatives name
an invariant tuple, and `mem_of_isExact5W` turns that into a member index. -/
theorem exists_end5W {M : Magma (Fin 5)} (hE : ∀ j, M.IsEndo (endE5W j))
    (hX : ∀ j, ¬ M.IsEndo (endX5W j)) : ∃ i, end5W i = M := by
  have hP : ∀ i, M.IsEndo (permE5W i) := by
    intro i; rw [permE5W_eq_endE5W]; exact hE (uidx5W i)
  have hop : M.op = Magma.transport permE5W tr5W (fun i ↦ M.op (rep5W i).1 (rep5W i).2) :=
    Magma.op_eq_transport (rep := rep5W) hP (by decide)
  obtain ⟨a, ha⟩ := Magma.exists_val_of_isEndo
    (E := permE5W) (idx := stabIdx5W0) (a := (rep5W 0).1) (b := (rep5W 0).2)
    (val := val5W0) hP (by decide) (by decide)
  obtain ⟨b, hb⟩ := Magma.exists_val_of_isEndo
    (E := permE5W) (idx := stabIdx5W1) (a := (rep5W 1).1) (b := (rep5W 1).2)
    (val := val5W1) hP (by decide) (by decide)
  obtain ⟨c, hc⟩ := Magma.exists_val_of_isEndo
    (E := permE5W) (idx := stabIdx5W2) (a := (rep5W 2).1) (b := (rep5W 2).2)
    (val := val5W2) hP (by decide) (by decide)
  obtain ⟨d, hd⟩ := Magma.exists_val_of_isEndo
    (E := permE5W) (idx := stabIdx5W3) (a := (rep5W 3).1) (b := (rep5W 3).2)
    (val := val5W3) hP (by decide) (by decide)
  obtain ⟨e, he⟩ := Magma.exists_val_of_isEndo
    (E := permE5W) (idx := stabIdx5W4) (a := (rep5W 4).1) (b := (rep5W 4).2)
    (val := val5W4) hP (by decide) (by decide)
  obtain ⟨f, hf⟩ := Magma.exists_val_of_isEndo
    (E := permE5W) (idx := stabIdx5W5) (a := (rep5W 5).1) (b := (rep5W 5).2)
    (val := val5W5) hP (by decide) (by decide)
  obtain ⟨g, hg⟩ := Magma.exists_val_of_isEndo
    (E := permE5W) (idx := stabIdx5W6) (a := (rep5W 6).1) (b := (rep5W 6).2)
    (val := id) hP (by decide) (by decide)
  obtain ⟨h, hh⟩ := Magma.exists_val_of_isEndo
    (E := permE5W) (idx := stabIdx5W7) (a := (rep5W 7).1) (b := (rep5W 7).2)
    (val := val5W7) hP (by decide) (by decide)
  obtain ⟨i, hi⟩ := Magma.exists_val_of_isEndo
    (E := permE5W) (idx := stabIdx5W8) (a := (rep5W 8).1) (b := (rep5W 8).2)
    (val := val5W8) hP (by decide) (by decide)
  obtain ⟨j, hj⟩ := Magma.exists_val_of_isEndo
    (E := permE5W) (idx := stabIdx5W9) (a := (rep5W 9).1) (b := (rep5W 9).2)
    (val := val5W9) hP (by decide) (by decide)
  have hdec : dec5W a b c d e f g h i j = fun i ↦ M.op (rep5W i).1 (rep5W i).2 := by
    ...
    · exact hi
    · exact hj
  have key : tab5W a b c d e f g h i j = M.op := by
    rw [tab5W_eq_transport, hdec, ← hop]
  have hok : Magma.isExact (tab5W a b c d e f g h i j) endE5W endX5W = true := by
    rw [key, Magma.isExact_iff]; exact ⟨hE, hX⟩
  obtain ⟨idx, hidx⟩ := mem_of_isExact5W a b c d e f g h i j hok
...
monoid is exactly the order-`10` monoid of `Magma.endE5W`, and no such magma satisfies `L`, then `L`
is not term-structural from `L'`, not even over finite carriers. -/
theorem not_termStructuralFromFin_of_end5W {L L' : Law.NatMagmaLaw} {i : Fin 1}
    (hsrc : @satisfies _ (Fin 5) (Magma.end5W i) L')
    (htgt : ∀ j, ¬ @satisfies _ (Fin 5) (Magma.end5W j) L) :
    ¬ L.TermStructuralFromFin L' := by
  intro h
  obtain ⟨M', hM', hfwd, hback⟩ := h (Magma.end5W i) hsrc
  obtain ⟨j, hM⟩ := Magma.exists_end5W
    (fun k ↦ (Magma.end5W_isEndo i k).of_termDefinable hfwd)
    (fun k hn ↦ Magma.end5W_not_isEndo i k (hn.of_termDefinable hback))
