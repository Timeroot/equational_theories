...
Now suppose that $G$ is also a field with a primitive fifth root of unity $\beta$ (so that $\beta = \left(\beta^{-2}\right)^2 = \beta^4$ is a square), but $-1$ and $\beta+1$ are not squares (for instance, $G$ could be the field of size $31$, with $\beta=2$).  If we define $x \op y = x - \beta(y-x)$, and then define $\op_{x,y}$ to be $\op^0$ when $y=x$, $\op^+$ when $y-x$ is a non-square, and $\op^-$ when $y-x$ is a non-square, one can then check that \eqref{ext-eq} holds. Since $\op^0$ is not right-cancellative, this gives a finite 677 magma that is not right-cancellative.
