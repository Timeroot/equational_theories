...
- `n=36`, `r=4`, degrees contained in `{4,6,9}`, and `|Z|≤12`;
- `n=36`, `r=3`, degrees contained in `{3,4,6,9,12}`, and `|Z|≤6`.

The [central-defect gap theorem](1485_small_central_defects.md) excludes
every defect strictly between zero and `r`, so the current script uses
the tighter cap `r²−r`. The historical timings below predate that theorem and used
