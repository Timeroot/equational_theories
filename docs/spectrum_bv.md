...
## Additional exhaustive certificates

`SmallCertificates.lean` imports 24 further exclusions generated by
`scripts/spectrum_small_certificates.py`:

| Order | Laws excluded |
| --- | --- |
| 4 | E467, E704, E907, E1076, E1279, E1286, E1313, E1489, E1516 |
| 5 | E907, E1483 |
| 6 | E63, E467, E670, E704, E907, E1076, E1110, E1279, E1286, E1313, E1483, E1516 |
| 7 | E1076 |

`FiniteTableEncoding.lean` packs the complete multiplication table into rows,
clips unused output codes, and proves `encoded_eq` for every magma of the
specified size. Each `Generated.Exclusion<L>_<n>` module instantiates the law at
every assignment, refutes the resulting conjunction with `bv_decide`, and lifts
that checked refutation back to `¬ Law<L>.HasModel <n>`. All encoding and LRAT
native checks are explicitly registered, and each final exclusion has a
`spectrum_assert ... complete` check. No solver answer alone is accepted.

For E1483 at order six, `SmallModelNormalization.lean` proves that a model can
be relabeled so that `0 ◇ 0` is either 0 or 1. If it is not 0, swap its value
with 1 while fixing 0; relabeling preserves every equation. This additional
constraint makes the checked refutation tractable. The individual probes took
about 6–75 seconds including elaboration and checking; generated certificates
allow a larger SAT timeout to accommodate concurrent project builds.

The generator records only completed cases. Attempts on E63/10, E670/7,
E704/9, E1279/9, and E1483/7 did not yet produce checked certificates; those
obligations retain their pending status in the catalogue.
