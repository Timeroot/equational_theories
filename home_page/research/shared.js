    ...
    pending.set(
      ...
      fetch(new URL(`data/${file}.json`, import.meta.url))
        .then((r) => {
          if (!r.ok)
            throw Error(
              `Could not load ${file}.json (${r.status}). The website data must be generated by the publishing workflow.`,
            );
          return r.json();
        })
        .catch((e) => {
